...
4. Start every commit subject with a reference such as `#3`.
5. Use the commit subject format `#<ref> <type>(<scope>): <summary>` or `#<ref> <type>: <summary>`.
6. Keep `TASK.md`, `PLANNING.md`, and `CHANGELOG.md` aligned to the same active reference number.
7. Prefer `npm run githelper -- commit --ref <n> --type <type> --scope <scope> --summary "<text>"` for final commit creation.
...
4. Ensure `npm run githelper -- check` passes before pushing a review branch.
