...
`MasterList` supports `table` and `cards` variants. Both show configured fields and optional view, edit, and delete actions. The table uses the shared Data Table block for search, sorting, and pagination. `MasterForm` supports `page` and `panel` layouts, controlled string values, and required-field checks.
