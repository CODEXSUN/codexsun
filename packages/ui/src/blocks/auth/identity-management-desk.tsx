...
import { KeyRoundIcon, PauseCircleIcon, PencilIcon, ShieldCheckIcon, Trash2Icon, UsersIcon } from "lucide-react";
import { MainWorkspace, type MdiNavigationSection } from "../../layouts/main-workspace";
import { Button } from "../../components/button";
import { Input } from "../../components/input";
import { FormBlock } from "../form";
import { createDataTableColumnHelper, DataTableBlock, DataTableFilterMenu, DataTableRowActions } from "../table";
import type { AuthenticatedRequest } from "./session-boundary";

type IdentityUser = { id: string; login: string; name: string; protected: boolean; roles: string[]; state: "active" | "disabled"; username: string };
...
function UsersPage({ applicationId, request }: { applicationId: string; request: AuthenticatedRequest }) {
  ...
  const [stateFilter, setStateFilter] = useState<"all" | IdentityUser["state"]>("all");
  ...
  const visibleUsers = stateFilter === "all" ? users : users.filter((user) => user.state === stateFilter);
  const suspend = (user: IdentityUser) => void request(apiPath(applicationId, `users/${user.id}`), { body: JSON.stringify({ login: user.login, name: user.name, state: "disabled", username: user.username }), headers: { "content-type": "application/json" }, method: "PUT" }).then(async (response) => { if (!response.ok) throw new Error((await response.json().catch(() => ({})) as { error?: string }).error ?? "Could not suspend user."); reload(); }).catch((reason) => setError(messageOf(reason)));
  const forceDelete = (user: IdentityUser) => void request(apiPath(applicationId, `users/${user.id}`), { method: "DELETE" }).then(async (response) => { if (!response.ok) throw new Error((await response.json().catch(() => ({})) as { error?: string }).error ?? "Could not delete user."); reload(); }).catch((reason) => setError(messageOf(reason)));
  return <DataTableBlock columns={[column.accessor("name", { header: "Name", cell: ({ getValue, row }) => <span className="inline-flex items-center gap-1.5">{getValue()}{row.original.protected ? <ShieldCheckIcon aria-label="Default protected record" className="size-4 text-amber-600" /> : null}</span> }), column.accessor("username", { header: "Username" }), column.accessor("login", { header: "Email" }), column.accessor("roles", { header: "Roles", cell: ({ getValue }) => getValue().join(", ") || "—" }), column.accessor("state", { header: "State" }), column.display({ id: "actions", header: "Actions", cell: ({ row }) => row.original.protected ? <Button aria-label="Default protected record" disabled size="icon-sm" variant="outline"><ShieldCheckIcon className="text-amber-600" /></Button> : <DataTableRowActions label={`Actions for ${row.original.name}`} actions={[{ id: "edit", label: "Edit", icon: <PencilIcon />, onSelect: () => setEditing(row.original) }, { id: "suspend", label: "Suspend", icon: <PauseCircleIcon />, disabled: row.original.state === "disabled", onSelect: () => suspend(row.original), tone: "destructive" }, { id: "force-delete", label: "Force delete", icon: <Trash2Icon />, onSelect: () => forceDelete(row.original), separatorBefore: true, tone: "destructive" }]} /> })] as never} data={visibleUsers} description="Create, update, suspend, and remove application-local accounts." emptyMessage="No users found." getRowId={(user) => user.id} getSearchText={(user) => `${user.name} ${user.username} ${user.login} ${user.roles.join(" ")}`} primaryAction={<Button onClick={() => setEditing({ id: "", login: "", name: "", protected: false, roles: [], state: "active", username: "" })}>New user</Button>} summary={error ? <p className="text-sm text-destructive">{error}</p> : undefined} title="Users" toolbarFilters={<DataTableFilterMenu activeCount={stateFilter === "all" ? 0 : 1} onClear={() => setStateFilter("all")} onSelect={setStateFilter} options={[{ checked: stateFilter === "all", label: "All users", value: "all" }, { checked: stateFilter === "active", label: "Active", value: "active" }, { checked: stateFilter === "disabled", label: "Suspended", value: "disabled" }]} />} />;
}
