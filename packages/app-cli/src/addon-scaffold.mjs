...
export function createAddon(rootDir, options) {
  ...
  const packagePath = resolve(registry.root, "packages", "addons", id);
  if (existsSync(packagePath)) throw new Error(`Add-on package already exists: packages/addons/${id}.`);
  ...
  const addon = {
    ...
    id,
    label,
    owner: `packages/addons/${id}`,
    package: `@codexsun/addon-${id}`,
    providerId,
    purpose: `${label} business capability.`,
    areas: [],
    contracts: [`${id}.v1`],
    events: [],
    dependencies: [],
