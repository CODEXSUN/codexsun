...
This registry is the project-level catalog for installed applications, add-ons, and runtime profiles.
...
The registry owns composition metadata only. Application source remains under its manifest `owner` path: platform source uses `core/platforms`, business apps use `apps/<application>`, and developer tooling uses `devkits/<application>`. Reusable contracts remain in `packages`.
...
reference, MDI entry, module tests, and runtime profile selection.
...
generated application from its manifest owner path and removes its registry, profile, MDI,
