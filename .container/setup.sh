...
DEPLOY_TECHMEDIA="${DEPLOY_TECHMEDIA:-true}"
...
TECHMEDIA_DOMAIN="${TECHMEDIA_DOMAIN:-techmedia.in}"
...
TECHMEDIA_DB_NAME="${TECHMEDIA_DB_NAME:-techmedia_in_db}"
...
TECHMEDIA_DB_HOST="${TECHMEDIA_DB_HOST:-$DB_HOST_DEFAULT}"
...
TECHMEDIA_DB_PORT="${TECHMEDIA_DB_PORT:-$DB_PORT_DEFAULT}"
...
TECHMEDIA_DB_USER="${TECHMEDIA_DB_USER:-$DB_USER_DEFAULT}"
...
TECHMEDIA_DB_PASSWORD="${TECHMEDIA_DB_PASSWORD:-$DB_PASSWORD_DEFAULT}"
...
clean_install() {
  ...
  if [ -f "$REPO_ROOT/$TECHMEDIA_COMPOSE_FILE" ]; then
    "${COMPOSE_CMD[@]}" -f "$REPO_ROOT/$TECHMEDIA_COMPOSE_FILE" down --remove-orphans -v || true
  fi

  docker rm -f "$CODEXSUN_CONTAINER" "$TMNEXT_CONTAINER" "$TIRUPUR_CONTAINER" "$TECHMEDIA_CONTAINER" >/dev/null 2>&1 || true
...
ensure_databases() {
  ...
  if [ "$DEPLOY_TECHMEDIA" = "true" ]; then
    ensure_database "$TECHMEDIA_DB_HOST" "$TECHMEDIA_DB_PORT" "$TECHMEDIA_DB_USER" "$TECHMEDIA_DB_PASSWORD" "$TECHMEDIA_DB_NAME"
  fi
}
...
drop_databases() {
  ...
  if [ "$DEPLOY_TECHMEDIA" = "true" ]; then
    drop_database "$TECHMEDIA_DB_HOST" "$TECHMEDIA_DB_PORT" "$TECHMEDIA_DB_USER" "$TECHMEDIA_DB_PASSWORD" "$TECHMEDIA_DB_NAME"
  fi
}
...
if [ "$DEPLOY_TECHMEDIA" = "true" ] && ensure_compose_file "techmedia" "$TECHMEDIA_COMPOSE_FILE"; then
  deploy_site "techmedia.in" "$TECHMEDIA_COMPOSE_FILE" "$TECHMEDIA_CONTAINER" \
    "$TECHMEDIA_DOMAIN" "$TECHMEDIA_DB_HOST" "$TECHMEDIA_DB_PORT" "$TECHMEDIA_DB_USER" "$TECHMEDIA_DB_PASSWORD" "$TECHMEDIA_DB_NAME"
fi
