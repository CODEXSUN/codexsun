...
function createTempSvgPath(testOutputPath: string) {
  const svgPath = `${testOutputPath}-menu-editor-logo.svg`
  ...
  writeFileSync(
    ...
    [
      ...
      '<text x="28" y="74" font-size="44" fill="#123456">Menu Editor</text>',
      "</svg>",
    ].join(""),
    "utf8"
  )
...
test("menu editor can upload and publish the light logo svg", async ({ page }, testInfo) => {
  const uploadedSvgPath = createTempSvgPath(testInfo.outputPath("menu-editor-logo"))

  await loginAsAdmin(page, "/dashboard/apps/ecommerce/menu-editor")

  await expect(page).toHaveURL(/\/dashboard\/apps\/ecommerce\/menu-editor$/)
  await expect(page.getByText("Logo source and publish", { exact: true })).toBeVisible()
  ...
  await page.getByRole("button", { name: "Publish logo live" }).click()
