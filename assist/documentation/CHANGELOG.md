...
### [v 1.0.51] 2026-07-24 5:47 pm - Prevent repository text encoding corruption
...
- Wired the encoding gate into the normal repository check while retaining a focused environment
  check alias for maintenance workflows.
- Bumped the repository, Platform API, Platform Web, and lockfile versions to 1.0.51.
...
### [v 1.0.46] 2026-07-23 12:01 am - Establish strict deployment history and agent runbook
...
- Consolidated the sanitized fresh-install and safe-update agent prompts into
  `.container/setup.md` and removed the temporary `.container/notes.md` handoff file.
- Added an append-only, versioned `.container/deploy-log.md` with established deployment history,
  exact setup/update commands, a mandatory evidence template, and explicit bug/blocker tracking.
- Made deployment-log completion and publication a strict agent requirement for successful, failed,
  blocked, reinstalled, migrated, and rollback deployment attempts.
- Corrected manual VPS verification commands to explicitly select `.container/vps.env` instead of
  accidentally falling back to the local deployment environment file.
- Ordered deployment logging after the update command's clean-repository preflight so the required
  audit entry cannot block the safe fast-forward workflow.
...
### [v 1.0.44] 2026-07-22 9:46 pm - Enforce application-only container deployment boundaries
...
- Added mandatory `.container/AGENTS.md` deployment rules covering prerequisites, exact ownership,
  prohibited destructive Docker operations, VPS backup requirements, and post-rollout verification.
- Changed setup so shared MariaDB, Redis, and Media are bootstrapped only when all three containers
  are absent; partial or unhealthy infrastructure now stops for operator review.
- Changed Billing reinstall and update workflows to rebuild only Billing application services and
  connect them to the existing shared network without replacing shared containers or volumes.
- Corrected VPS routing documentation so `app.codexsun.com` belongs to Billing while the portfolio
  domains remain owned by CMS.
- Aligned package, workspace, container build, image, and deployment defaults to version 1.0.44.

## v-1.0.43

### [v 1.0.43] 2026-07-22 8:52 pm - Separate application stacks and production deployment

#### Database Changes

- Database update: Yes.
- Added deterministic provisioning for the CODEXSUN, Cotton, Sukraa, and Ganapathi tenant databases.
- Kept Platform, Core, Billing, and Mail migration/seed execution ordered through module-owned lifecycle contracts.

#### App Codebase Changes

- Separated Billing, CMS, and Techmedia runtime composition while retaining one shared MariaDB, Redis, media, and Docker network layer.
- Added source-built local/VPS installation, Traefik routing, container health checks, smoke tests, and optimized Docker build contexts.
- Connected Super Admin App Operations to repository application metadata and corrected production Query Client composition.
- Renamed the Billing runtime containers to `codexsun-billing-api` and `codexsun-billing-web` and bumped the repository to 1.0.43.
