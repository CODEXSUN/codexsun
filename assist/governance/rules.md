...
- CODEXSUN uses npm only. Run dependency commands from the current Git repository root; pnpm, Yarn, workspace-local installs, and alternative lockfiles are prohibited.
- Each sibling Git repository contains exactly one dependency installation tree at its root `node_modules`. Never create or retain `node_modules` inside that repository's npm workspaces or source folders.
