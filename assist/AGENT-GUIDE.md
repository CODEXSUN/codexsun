...
- Use npm only from the current Git repository root.
- Each Git repository keeps one root `node_modules`, one root `package-lock.json`, and one root `dist`.
...
- Do not create module-specific business behavior inside the sibling `ui` repository.
