...
- Core routes use the public Framework tenant-access contract, keeping the sibling `core` repository independent from Platform internals.
- Common records, contacts, companies, and products are isolated by the selected tenant database; those tables do not add redundant `tenant_id` columns.
