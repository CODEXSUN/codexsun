...
export const coreAuthOptionCatalogSeeder = defineDatabaseSeeder({
  id: "cxapp:auth:09-auth-option-catalog",
  appId: "cxapp",
  moduleKey: "auth",
  ...
  run: async ({ database }) => {
    ...
    await queryDatabase.deleteFrom(cxappTableNames.authOptionCatalog).execute()
    await queryDatabase
      .insertInto(cxappTableNames.authOptionCatalog)
      .values(
