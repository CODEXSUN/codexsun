...
The scripts read the application version from `apps/zuno/VERSION`. The human-readable version is also stored in the API and web image labels. Docker-safe image tags replace spaces with hyphens.
...
- Reads the current version from `apps/zuno/VERSION`.
