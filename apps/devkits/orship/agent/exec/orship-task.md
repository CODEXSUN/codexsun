...
The working folder is `apps/devkits/orship`.

The agent skill file is `apps/devkits/orship/agent/skills.md`.

The task file is `apps/devkits/orship/agent/exec/orship-task.md`.
...
Keep Orship product code inside `apps/devkits/orship`.
...
Do not create `node_modules` inside `apps/devkits/orship`.

Do not create local `dist` or `.turbo` folders inside `apps/devkits/orship`.
...
The API host is `apps/devkits/orship/api`.

The web host is `apps/devkits/orship/web`.
...
The Go Docker manager lives in `apps/devkits/orship/docker` and runs as a private service in the local compose stack.
...
1. Read `apps/devkits/orship/agent/skills.md`.
2. Read `apps/devkits/orship/README.md`.
3. Inspect `apps/devkits/orship/api` and `apps/devkits/orship/web`.
...
- Orship work stays inside `apps/devkits/orship` unless a reviewed contract change exists.
...
- The Orship container scaffold lives in `apps/devkits/orship/.container`.
- `docker-compose.yml` builds separate API and web targets from one Dockerfile.
- The API container owns the SQLite path at `/workspace/storage/apps/devkits/orship/private/data/orship_db.sqlite`.
...
- `apps/devkits/orship/VERSION` owns the Orship development version; the root `package.json` owns the global repository release version.
- The web status bar shows the Orship development version at its right edge.
- `orship-update.sh` reads `apps/devkits/orship/VERSION`, rebuilds, and recreates the local Docker stack for live updates.
