...
Orship owns orchestration product behavior inside `apps/devkits/orship`.
...
- `apps/devkits/orship/api` owns API routes, providers, configuration, health checks, and operational services.
- `apps/devkits/orship/web` owns the browser workspace and operator interface.
- `apps/devkits/orship/docker` owns the Go Docker manager used by the local container stack.
...
Orship reads application values from `apps/devkits/orship/api/.app.env` and `apps/devkits/orship/web/.app.env`.
...
Keep Orship changes inside `apps/devkits/orship` unless a public package contract needs a reviewed change.
...
`apps/devkits/orship/VERSION` owns the Orship development version. The web status bar
shows this value at its right edge. Local development and Docker builds use the
same source file.

Use `apps/devkits/orship/.container/orship-update.sh` to apply a local Docker update.
