...
export function InfrasWorkspace({ request, selectedUuid, view, onBack, onCreate, onSaved, onSelect }: InfrasWorkspaceProps) {
  ...
  if (view === "infras-show" && selectedUuid) {
    return <InfrasShowPage request={request} uuid={selectedUuid} onBack={onBack} />;
  }
...
function InfrasShowPage({ request, uuid, onBack }: { request: typeof fetch; uuid: string; onBack: () => void }) {
  ...
  return (
    <main className="size-full overflow-y-auto bg-background p-6">
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        ...
        {infra.data ? <InfraDetails infra={infra.data} request={request} /> : null}
      </section>
    </main>
  );
}
...
function InfraDetails({ infra, request }: { infra: OrshipInfraRecord; request: typeof fetch }) {
  ...
  const displayMetrics = snapshot.data ? liveMetrics(snapshot.data) : [];
  ...
  return (
    <div className="grid gap-6">
      ...
      {activeTab === "details" ? (
        ...
        <InfraTabScaffold tab={activeTab} request={request} />
      )}
    </div>
  );
}
...
function ContainerDetailsCard({ container, infra, snapshot }: { container?: DockerContainer; infra: OrshipInfraRecord; snapshot?: DockerContainerSnapshot }) {
  const metrics = snapshot ? liveMetrics(snapshot) : [];
...
function liveMetrics(snapshot: DockerContainerSnapshot): OrshipInfraMetric[] {
  ...
  return values.map(([label, value, percent]) => ({
    label,
    percent,
    series: percent === undefined ? [0] : [percent],
    value,
  }));
}
