...
process.chdir(resolve(root, 'apps/zuno/api'));
const {readConfig} = await import(pathToFileURL(resolve(root, 'apps/zuno/api/src/config.ts')).href);
...
if (mode === 'status') {
  ...
  identity.close();
  const {ZunoHandoffStore} = await import(pathToFileURL(resolve(root,'apps/zuno/api/src/modules/handoff/handoff-store.ts')).href);
  const {PortalStore} = await import(pathToFileURL(resolve(root,'apps/zuno/api/src/modules/cxforge/portal-store.ts')).href);
