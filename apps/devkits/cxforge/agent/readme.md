...
The scripts use the `cxforge` Compose project. They connect CXForge to the shared `codexsun-network` network.
...
```sh
sh apps/devkits/cxforge/.container/cxforge-setup.sh
```
...
3. Read the release number from `apps/devkits/cxforge/VERSION`.
...
```sh
CXFORGE_NO_CACHE=true sh apps/devkits/cxforge/.container/cxforge-setup.sh
```
...
The setup clones directly into `/workspace`. Later setup runs keep the existing repository registration and checkout. All tasks execute serially in this checkout. No mirror or workspace volume is required.

For a complete project setup, set `CXFORGE_SETUP_RECIPE` to a setup JSON file before running `cxforge-setup.sh`.
The script starts the container and submits the recipe. Go clones, configures, installs, migrates, and starts the preview.
Use `cxforge-workspace.sh setup.json` to submit a recipe to an existing container.
See `workspace-setup.md` for the request fields. Keep secret-bearing recipe files outside Git tracking.
The first preview port is 7300 by default. Worker state and caches stay outside the checkout in `/var/lib/cxforge`.
The update script refuses legacy `/workspace/repo` containers. Preserve them before creating a fresh flat workspace.
...
```sh
sh apps/devkits/cxforge/.container/cxforge-update.sh
```
...
- The release number from `apps/devkits/cxforge/VERSION`.
- The first 12 characters of the current Git commit SHA.
- The `-dirty` suffix when `apps/devkits/cxforge` has uncommitted changes.
...
```sh
sh apps/devkits/cxforge/.container/cxforge-drop.sh
```
...
- The `cxforge` Compose project resources.
...
```sh
CXFORGE_CONFIRM_DROP=yes sh apps/devkits/cxforge/.container/cxforge-drop.sh
```
...
- Setup: `apps/devkits/cxforge/.container/cxforge-setup.sh`
- Update: `apps/devkits/cxforge/.container/cxforge-update.sh`
- Drop: `apps/devkits/cxforge/.container/cxforge-drop.sh`
- Compose: `apps/devkits/cxforge/.container/compose.yml`
- Release number: `apps/devkits/cxforge/VERSION`
