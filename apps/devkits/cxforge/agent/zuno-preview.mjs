...
for (const file of await readdir(`${root}/registry/applications`)) {
  if (!file.endsWith('.json')) continue;
  const application = JSON.parse(await readFile(`${root}/registry/applications/${file}`, 'utf8'));
...
const api = spawn(`${root}/node_modules/.bin/tsx`,['src/server.ts'],{...options,cwd:`${root}/apps/zuno/api`});
let web;
let stopping = false;
...
web=await createServer({root:`${root}/apps/zuno/web`,configFile:`${root}/apps/zuno/web/vite.config.ts`,base:'/',
