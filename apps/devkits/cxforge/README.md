...
Local Docker now defaults to `tools` mode. Zuno sends explicit commands, and CXForge executes them without a model.
See [the command API](agent/tools-api.md) and the typed client in `api/client.ts`.
The model-driven task flow below is retained for deployments that explicitly select model mode.
...
CXForge stores task state in `/var/lib/cxforge/state.json`. All tasks use `/workspace`. Multiple tasks and follow-up commands share the files, branch, dependencies, and preview. Use another container for parallel work or another repository.
...
Go uses `/var/lib/cxforge/cache/go-build` and `/var/lib/cxforge/cache/go-mod`. Keep worker state and caches writable.

CXForge stores one direct checkout at `/workspace` for all commands in this container.
...
Registration clones directly into `/workspace` and records the initial commit SHA. A second repository requires another container.
...
```sh
docker compose -f apps/devkits/cxforge/.container/docker-compose.vps.yml up -d
```
...
```sh
sh apps/devkits/cxforge/.container/publish.sh
```

Set `CXFORGE_PUSH=true` to push both the version from `apps/devkits/cxforge/VERSION` and the `latest` tag. The VPS Compose file defaults to the immutable versioned tag.

After deployment, run the authenticated smoke checks:

```sh
CXFORGE_ZUNO_CLIENT_KEY=<secret> sh apps/devkits/cxforge/.container/production-check.sh
```
