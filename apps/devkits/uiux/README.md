...
- `apps/devkits/uiux/web` owns the gallery pages, catalogs, previews, specimens, examples,
...
a database provider, or an API only to match other applications. If a future
approved UIUX module needs durable data, its application composition root must
create and inject the required typed Provider under the Provider Standard.
...
- `apps/devkits/uiux/web/.app.env` owns `WEB_HOST` and `WEB_PORT`.
...
See the [module registry](../../assist/modules/registry.md) and the
