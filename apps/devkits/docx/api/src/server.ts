...
import { DocxFoundationProvider } from "./modules/foundation/provider.js";

const config = readConfig();
const identity = new LocalIdentityStore(config);
await identity.initialize();
const provider = new DocxFoundationProvider();
const profile = readApplicationDeployableProfile({ applicationId: "docx", availableProviderIds: ["platform.core", provider.manifest.id] });
...
await app.register(cors, { origin: process.env.DOCX_WEB_ORIGIN, methods: ["GET", "HEAD", "OPTIONS", "POST", "PUT", "DELETE"], allowedHeaders: ["Authorization", "Content-Type", "X-Codexsun-Browser-Session", "X-Codexsun-Auto-Login-Desk"] });
await app.register(swagger, { openapi: { info: { title: "DOCX API", version: "1.0.0" }, openapi: "3.0.3" }, transform: jsonSchemaTransform });
...
app.post("/api/v1/docx/auth/login", { schema: { body: identityLoginSchema, response: { 200: identityLoginResponseSchema, 400: identityErrorResponseSchema, 401: identityErrorResponseSchema, 429: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/docx/auth/:portal/login", { schema: { params: z.object({ portal: z.enum(["admin", "super-admin"]) }), body: identityLoginSchema, response: { 200: identityLoginResponseSchema, 400: identityErrorResponseSchema, 401: identityErrorResponseSchema, 429: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/docx/auth/password-reset/request", { schema: { body: identityPasswordResetRequestSchema, response: { 202: identityPasswordResetAcceptedSchema } } }, async (request, reply) => {
...
app.post("/api/v1/docx/auth/password-reset/confirm", { schema: { body: identityPasswordResetConfirmationSchema, response: { 204: z.null(), 400: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/docx/auth/development-login", async (request, reply) => {
  const browserSessionId = identityBrowserSessionIdSchema.safeParse(request.headers["x-codexsun-browser-session"]);
...
function isPublicPath(url: string): boolean {
  ...
  return path === "/api/v1/docx/auth/login"
    || path === "/api/v1/docx/auth/admin/login"
    || path === "/api/v1/docx/auth/super-admin/login"
    || path === "/api/v1/docx/auth/development-login"
    || path === "/api/v1/docx/auth/password-reset/request"
    || path === "/api/v1/docx/auth/password-reset/confirm"
