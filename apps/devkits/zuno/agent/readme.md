...
The scripts read the application version from `apps/devkits/zuno/VERSION`. The human-readable version is also stored in the API and web image labels. Docker-safe image tags replace spaces with hyphens.
...
```sh
sh apps/devkits/zuno/.container/zuno-setup.sh
```
...
```sh
sh apps/devkits/zuno/.container/zuno-update.sh
```

The update script:

- Reads the current version from `apps/devkits/zuno/VERSION`.
...
```sh
sh apps/devkits/zuno/.container/zuno-drop.sh
```
