...
export interface DroppedServerRecord {
  id: string;
  name: string;
  apiUrl: string;
  containerId: string;
...
export interface WorkspaceProfileRecord {
  ...
  migrationStatusCommand: string;
  migrateCommand: string;
  migrationVerifyCommand: string;
...
export class PortalStore {
  ...
  removeLegacyServer(apiUrl: string): void {
    this.db.prepare("DELETE FROM portal_servers WHERE name=? AND url=?").run("Local CXForge", new URL(apiUrl).origin);
  }

  workspaceProfiles(): WorkspaceProfileRecord[] {
    return (this.db.prepare("SELECT value FROM portal_values WHERE key LIKE 'workspace-profile:%' ORDER BY key").all() as { value: string }[])
      .map((row) => JSON.parse(row.value) as WorkspaceProfileRecord)
      .sort((left, right) => left.name.localeCompare(right.name));
  }

  saveWorkspaceProfile(input: Omit<WorkspaceProfileRecord, "id" | "savedAt">): WorkspaceProfileRecord {
    const profile: WorkspaceProfileRecord = { ...input, id: randomUUID(), savedAt: new Date().toISOString() };
    this.set(`workspace-profile:${profile.id}`, profile);
    return profile;
  }

  dropServer(serverId: string, input: Omit<DroppedServerRecord, "id" | "droppedAt">): DroppedServerRecord {
    this.server(serverId);
    const runtime = this.get<{ operationId?: string }>(`runtime:${serverId}`);
    const dropped: DroppedServerRecord = { id: serverId, ...input, droppedAt: new Date().toISOString() };
    this.db.exec("BEGIN IMMEDIATE");
    try {
      this.db.prepare("INSERT OR REPLACE INTO portal_values VALUES(?,?)").run(`dropped:${serverId}`, JSON.stringify(dropped));
      this.db.prepare("DELETE FROM portal_servers WHERE id=?").run(serverId);
      this.db.prepare("DELETE FROM portal_events WHERE server=?").run(serverId);
      this.db.prepare("DELETE FROM portal_commands WHERE key LIKE ?").run(`${serverId}:%`);
      this.db.prepare("DELETE FROM portal_values WHERE key=?").run(`runtime:${serverId}`);
      this.db.prepare("DELETE FROM portal_values WHERE key=?").run(`snapshot:${serverId}`);
      if (runtime?.operationId) this.db.prepare("DELETE FROM portal_values WHERE key=?").run(`provision-state:${runtime.operationId}`);
      this.db.exec("COMMIT");
      return dropped;
    } catch (error) {
