...
const connectionUrl = process.env.ORSHIP_MARIADB_INTEGRATION_URL;

test("runs orship migrations against an explicit MariaDB test database", { skip: !connectionUrl }, async () => {
  ...
  try {
    ...
    await runner.run({ moduleId: "orship.integration", migrations: [], seeders: [] });
  } finally {
