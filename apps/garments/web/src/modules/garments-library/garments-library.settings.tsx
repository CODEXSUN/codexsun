import type { DocumentationScanResponse } from "@codexsun/garments-contracts";
import { Badge } from "@codexsun/ui/components/badge";
import { Button } from "@codexsun/ui/components/button";
import { ArrowLeft, FileSearch, FileWarning, RefreshCw, TextSearch } from "lucide-react";
import { useEffect, useState } from "react";
import { scanDocumentation } from "./garments-library.services";

export function GarmentsLibrarySettings({ onBack }: { onBack: () => void }) {
  ...
  const issueCount = (result?.missingDocumentation.length ?? 0) + (result?.unorganizedFiles.length ?? 0);

  return (
    <section className="size-full overflow-y-auto bg-background" aria-labelledby="garments-settings-title">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-6 py-10 lg:px-10">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            <Button aria-label="Back to documentation" size="icon" variant="outline" onClick={onBack}>
              <ArrowLeft />
            </Button>
            <div>
              <p className="text-sm font-medium text-muted-foreground">Settings</p>
              <h1 id="garments-settings-title" className="text-2xl font-semibold tracking-tight">
        ...
        {result ? (
          <>
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <Metric icon={FileWarning} label="Missing READMEs" value={result.missingDocumentation.length} />
              <Metric icon={TextSearch} label="Unorganized files" value={result.unorganizedFiles.length} />
            </div>
            <p className="text-sm text-muted-foreground">Last scanned {new Date(result.scannedAt).toLocaleString()}.</p>
            <IssueList
...
function Metric({ icon: Icon, label, value }: { icon: typeof FileSearch; label: string; value: number }) {
  return (
