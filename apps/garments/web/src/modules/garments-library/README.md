# Garments Library Web Module

## Purpose

Displays the Garments API library in a responsive, keyboard-operable MDI workspace.

## Identity and version

- Module ID: `garments.library.web`
- Version: `0.1.2`
- Scope: `garments`
- Status: `active`
...
- Routes and UI paths: the Garments web root.
...
- Editor: the header Edit action opens a Garments-owned drill-down upsert page with an authoritative Markdown mode, explicit Save and Back controls, and a stale-source conflict message.
...
- Reader styling: GitHub-Flavored Markdown tables use a scrollable table surface. Inline snippets use compact muted pills. Fenced code samples use highlighted, copyable, high-contrast surfaces. Mermaid flowchart fences render as safe Garments-owned SVG diagrams.
- Article assets: Markdown image paths resolve through the Garments API. Store repository-owned images below an `assets` folder near their articles. See [article assets](../../../../../../assist/assets/README.md).
- Settings: `VITE_GARMENTS_API_URL` selects the Garments API. Define it from the root `.env`; no application fallback is provided. The shared Settings entry opens the read-only Documentation health scan. It reports missing README files and unorganized Markdown, MDX, and text sources without moving or changing them.

## Public contracts and verification

- API: consumes `@codexsun/garments-contracts` only through the Garments HTTP API.
- Dependencies: shared `@codexsun/ui` layout, Table primitive, and versioned Garments API routes.
- Browser verification needs both Garments runtimes running locally.
...
- [Documentation Workspace layout](../../../../../../assist/records/garments/2026-09-09-documentation-workspace-layout.md)
