...
import { getDocumentNavigationLabel, getGarmentsIndexGroups } from "./garments-library.index";

export function GarmentsIndexPage({
  documents,
  onSelect,
}: {
  ...
  const groups = getGarmentsIndexGroups(documents);
  ...
  return (
    <article className="w-full">
      ...
      <p className="mt-3 max-w-3xl text-muted-foreground">
        Browse repository guidance by Assist area, application, module, shared package, and runtime owner.
      </p>
          ...
          <section key={group.id}>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {group.documents.map((document) => (
                ...
                >
                  <span className="font-medium group-hover:text-primary">{getDocumentNavigationLabel(document)}</span>
                  <span className="mt-2 line-clamp-2 text-sm text-muted-foreground">
                    {document.description || document.path}
                  </span>
                  <code className="mt-auto pt-3 text-xs text-muted-foreground">{document.path}</code>
                </Button>
              ))}
            </div>
          </section>
...
function IndexMetric({ icon: Icon, label, value }: { icon: typeof FileText; label: string; value: number | string }) {
  return (
