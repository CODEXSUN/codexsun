...
import { fetchDocument, fetchDocuments, updateDocument } from "./garments-library.services";
import type { GarmentsState } from "./garments-library.types";

const initialState: GarmentsState = { documents: [], loading: true };

export function useGarmentsLibrary() {
  const [state, setState] = useState<GarmentsState>(initialState);
  const [selectedSlug, setSelectedSlug] = useState(readSlugFromLocation);
  const documentCache = useRef(new Map<string, GarmentsState["activeDocument"]>());
  ...
  useEffect(() => {
    ...
    void fetchDocuments(controller.signal)
      ...
      .catch(
        (error: unknown) =>
          !isAbortError(error) && setState({ documents: [], error: toMessage(error), loading: false }),
      );
  ...
  useEffect(() => {
    ...
    void fetchDocument(selectedSlug, controller.signal)
      ...
      .catch(
        (error: unknown) =>
          !isAbortError(error) && setState((current) => ({ ...current, error: toMessage(error), loading: false })),
      );
    return () => controller.abort();
  }, [selectedSlug]);
...
function toMessage(error: unknown): string {
  return error instanceof Error ? error.message : "Garments is unavailable.";
}
