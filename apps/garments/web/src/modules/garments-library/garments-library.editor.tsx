...
import type { Document } from "@codexsun/garments-contracts";
...
export function GarmentsLibraryEditor({
  document,
  onBack,
  onSave,
}: {
        ...
        <div className="mx-auto flex w-4/5 max-w-[120rem] flex-col gap-6 py-8">
          ...
          <div className="flex items-center justify-between gap-3 border-b">
            <div className="flex items-center gap-1" role="tablist" aria-label="Editor mode">
              <EditorModeButton active={mode === "write"} icon={FilePenLine} onClick={() => setMode("write")}>
                Write
              </EditorModeButton>
              <EditorModeButton active={mode === "markdown"} icon={Code2} onClick={() => setMode("markdown")}>
                Markdown
              </EditorModeButton>
            </div>
            <span className="hidden text-sm text-muted-foreground sm:block">Markdown is the saved source format</span>
          </div>
