...
import { IdeasArchitectureStandards } from "./ideas.architecture";
import { IdeasMonorepo } from "./ideas.monorepo";
import { getIdeasPage, type IdeasPageId } from "./ideas.types";

const deliveryStages = [
  ["1", "Durable module runtime", "Installed-module state and a checksum-protected migration ledger."],
  ["2", "Request and diagnostics", "Request context, cancellation, readiness details, and lifecycle diagnostics."],
  [
    "3",
    "Identity extension points",
    "Identity as an application module with named authentication and policy boundaries.",
  ],
  ["4", "Events and background work", "Typed events first; durable outbox and jobs only for a real consumer."],
  ["5", "Client platform adapters", "Shared contracts before Tauri or Expo adapters are approved."],
] as const;

export function IdeasWorkspace({ page }: { page: IdeasPageId }) {
  const topology = useMdiTopology();
  const idea = getIdeasPage(page);

  return (
    <TopologyRegion as="div" className="flex size-full min-h-0 flex-col" id="11" topology={topology}>
      <GarmentsLibraryHeader document={idea} />
      <div className="garments-reader-scroll flex-1 overflow-y-auto">
        {page === "architecture" ? <IdeasArchitectureStandards /> : null}
        {page === "monorepo" ? <IdeasMonorepo /> : null}
        {page === "plan" ? <DevelopmentPlan /> : null}
      </div>
    </TopologyRegion>
  );
}
    ...
    <div className="mx-auto w-4/5 max-w-[120rem] px-6 py-10 lg:px-10">
      <TopologyRegion className="max-w-4xl" id="11.1" topology={topology}>
        ...
        <p className="mt-4 max-w-3xl text-base leading-7 text-muted-foreground">
          CODEXSUN is a modular application platform. The framework remains small and runtime-neutral; Platform Core
          supplies reusable technical contracts; applications select infrastructure; business modules own their domain
          behavior.
        </p>
      </TopologyRegion>
      ...
      <TopologyRegion className="mt-10" id="11.2" topology={topology}>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] lg:items-center">
          <FlowStep icon={ClipboardCheckIcon} title="Real consumer" detail="A named product need" />
          <FlowArrow />
          <FlowStep icon={BlocksIcon} title="Owned contract" detail="Stable and business-free" />
          <FlowArrow />
          <FlowStep icon={NetworkIcon} title="Application proof" detail="Composition and failure flow" />
          <FlowArrow />
          <FlowStep icon={ShieldCheckIcon} title="Verified release" detail="Tests, garments, and lifecycle" />
        </div>
      </TopologyRegion>
      ...
      <TopologyRegion className="mt-12 border-t pt-6" id="11.5" topology={topology}>
        ...
        <p className="mt-3 max-w-4xl leading-7 text-muted-foreground">
          No reflection-based dependency injection, service locator, generic CRUD engine, or business behavior in shared
          packages. A shared capability must have an owner, a real consumer, explicit failure behavior, and focused
          verification.
        </p>
      </TopologyRegion>
    </div>
...
function FlowStep({ detail, icon: Icon, title }: { detail: string; icon: typeof BlocksIcon; title: string }) {
  return (
...
function PatternCard({ detail, icon: Icon, title }: { detail: string; icon: typeof BlocksIcon; title: string }) {
  return (
    <Card>
      <CardHeader>
        <Icon className="size-5 text-muted-foreground" />
