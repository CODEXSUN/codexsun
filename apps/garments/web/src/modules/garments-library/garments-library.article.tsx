...
import { renderMermaidFlowcharts } from "./garments-library.mermaid";
import { getDocumentAssetUrl } from "./garments-library.services";
import { getDocumentHeadings } from "./garments-library.utils";

export function GarmentsLibraryArticle({ html, path, source }: { html: string; path: string; source: string }) {
...
function addCodeControls(content: HTMLDivElement) {
  content.querySelectorAll("pre").forEach((pre) => {
    if (pre.dataset.garmentsCodeReady) return;
    const code = pre.querySelector("code");
    if (!code || isMermaidBlock(code)) return;

    pre.dataset.garmentsCodeReady = "true";
...
function createLanguageLabel(language: string): HTMLSpanElement {
  ...
  label.className = "garments-code-language";
...
function createCopyButton(code: HTMLElement): HTMLButtonElement {
  ...
  button.className = "garments-code-copy";
...
function getCodeLanguage(code: Element): string | undefined {
  return [...code.classList].find((value) => value.startsWith("language-"))?.slice("language-".length);
}
