...
export function renderMermaidFlowcharts(content: HTMLDivElement) {
  content.querySelectorAll("pre > code.language-mermaid").forEach((code) => {
    ...
    error.className = "garments-mermaid-error";
    error.textContent = "Use Mermaid flowchart syntax such as “flowchart LR” and “A[Start] --> B[End]”.";
...
function createFlowchart(source: string): HTMLDivElement | undefined {
  ...
  const edges = lines.map((line) => parseEdge(line, nodes)).filter((edge): edge is MermaidEdge => Boolean(edge));
  if (!edges.length) return undefined;

  const wrapper = document.createElement("div");
  wrapper.className = "garments-mermaid-diagram";
...
function createSvg(nodes: Map<string, MermaidNode>, edges: MermaidEdge[], direction: string): SVGSVGElement {
...
function getPositions(lanes: Map<number, MermaidNode[]>, horizontal: boolean) {
  ...
  [...lanes.entries()].forEach(([lane, laneNodes]) => {
    laneNodes.forEach((node, index) => {
      positions.set(
        node.id,
        horizontal ? { x: 36 + lane * 268, y: 36 + index * 96 } : { x: 36 + index * 268, y: 36 + lane * 96 },
      );
    });
  });
  return positions;
...
function createArrowMarker(): SVGDefsElement {
  ...
  marker.id = "garments-mermaid-arrow";
...
function appendEdge(
  ...
  line.setAttribute("marker-end", "url(#garments-mermaid-arrow)");
  ...
  svg.append(line);
  if (edge.label) svg.append(createText(edge.label, (from.x + to.x + 172) / 2, (from.y + to.y + 60) / 2 - 8, "11"));
}

function appendNode(svg: SVGSVGElement, node: MermaidNode, position: { x: number; y: number } | undefined) {
  if (!position) return;
