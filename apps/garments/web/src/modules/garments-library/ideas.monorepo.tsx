...
import { monorepoApplications, monorepoPractices, monorepoSharedPackages } from "./ideas.monorepo.data";
  ...
  return (
    <div className="mx-auto w-4/5 max-w-[120rem] px-6 py-10 lg:px-10">
      <TopologyRegion className="max-w-4xl" id="11.11" topology={topology}>
        ...
        <h1 className="mt-5 text-3xl font-bold tracking-tight">Shared packages and application ownership</h1>
        <p className="mt-4 max-w-3xl text-base leading-7 text-muted-foreground">
          CODEXSUN keeps reusable technical capabilities in shared packages and product behavior in source-owned
          applications. One repository gives teams a common foundation without merging every application into one
          product.
        </p>
      </TopologyRegion>
      ...
      <TopologyRegion className="mt-10" id="11.12" topology={topology}>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Shared packages sit between package code and application code. Applications use public package exports.
        </p>
      ...
      <TopologyRegion className="mt-12" id="11.18" topology={topology}>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
          Your domain sends visitors to a public server. The repository builds the web and API services that the server
          runs.
        </p>
        <div className="mt-5 rounded-xl border bg-muted/30 p-5 sm:p-7">
          <MonorepoFlowLane
      ...
      <TopologyRegion className="mt-12" id="11.13" topology={topology}>
        ...
        <p className="mt-2 max-w-4xl text-sm leading-6 text-muted-foreground">
          <code>packages/ui</code> is the visual foundation for every browser application. It owns reusable presentation
          behavior, not business screens, workflows, or application data.
        </p>
...
function SharedUiCard({ detail, icon: Icon, title }: { detail: string; icon: typeof PaletteIcon; title: string }) {
  return (
    <WorkspaceActionCard
