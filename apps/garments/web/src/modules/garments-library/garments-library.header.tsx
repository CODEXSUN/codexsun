...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@codexsun/ui/components/tooltip";

type GarmentsHeaderDocument = {
  path: string;
  title: string;
};

export function GarmentsLibraryHeader({
  document,
  onEdit,
  title,
}: {
  document?: GarmentsHeaderDocument;
  onEdit?: () => void;
  title?: string;
}) {
  const [actionState, setActionState] = useState<"failed" | "idle" | "path-copied" | "shared">("idle");
  ...
  return (
    <header className="flex h-12 shrink-0 items-center justify-between gap-3 border-b bg-background/95 px-4 backdrop-blur lg:px-6">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <p className="truncate text-sm font-semibold">{document?.title ?? title ?? "Documentation index"}</p>
      </div>
      {document ? (
