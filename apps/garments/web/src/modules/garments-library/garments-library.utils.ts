...
export function getRelatedDocuments(document: Document, documents: DocumentSummary[]): DocumentSummary[] {
...
export function matchesDocument(document: DocumentSummary, query: string): boolean {
  ...
  return [document.title, document.description, document.path, ...document.tags, ...document.aliases]
    .join(" ")
    .toLocaleLowerCase()
    .includes(normalized);
}
