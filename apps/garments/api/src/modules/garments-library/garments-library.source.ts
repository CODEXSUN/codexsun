...
import type { DocumentationIssue, DocumentationScanResponse } from "@codexsun/garments-contracts";
...
const ignoredDirectories = new Set([".git", ".obsidian", "coverage", "dist", "node_modules", "storage"]);
...
export type GarmentsAsset = {
  content: Buffer;
  contentType: string;
};

export class GarmentsVault {
  public constructor(
  ...
  public async update(
    ...
    if (document.sourceHash !== input.sourceHash) {
      throw new GarmentsDocumentConflictError();
    }
  ...
  public async getAsset(assetPath: string): Promise<GarmentsAsset | undefined> {
  ...
  private async readDocument(sourcePath: string): Promise<DocumentRecord> {
    ...
    const title = this.readText(metadata.title) ?? this.readHeading(parsed.content) ?? this.toTitle(slug);
  ...
  private toSlug(sourcePath: string): string {
    ...
    return relative(root, sourcePath).replace(extname(sourcePath), "").replaceAll(sep, "/").toLowerCase();
  }
...
export class GarmentsDocumentConflictError extends Error {
  public constructor() {
