...
import { GarmentsDocumentConflictError } from "./garments-library.source.js";
...
export async function registerGarmentsLibraryRoutes(
  server: FastifyInstance,
  environment: GarmentsEnvironment,
  service: GarmentsLibraryService,
): Promise<void> {
  server.get("/api/garments/v1/documents", async () => {
    return documentListResponseSchema.parse({
      documents: await service.listDocuments(),
      mode: environment.GARMENTS_INDEX_MODE,
    });
  });

  server.get("/api/garments/v1/scan", async () => {
    return documentationScanResponseSchema.parse(await service.scanDocumentation());
  });

  server.get("/api/garments/v1/assets/*", async (request, reply) => {
  ...
  server.get("/api/garments/v1/documents/*", async (request, reply) => {
  ...
  server.put("/api/garments/v1/documents/*", async (request, reply) => {
    const { "*": slug } = documentParamsSchema.parse(request.params);
    ...
    try {
      ...
      if (error instanceof GarmentsDocumentConflictError) {
  ...
  server.post("/api/garments/v1/index/sync", async () => {
