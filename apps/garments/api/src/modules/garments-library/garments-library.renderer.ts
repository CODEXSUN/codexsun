...
export class GarmentsRenderer {
  public async render(document: DocumentRecord): Promise<string> {
    if (document.path.endsWith(".txt")) {
      return renderToStaticMarkup(
        createElement("pre", null, createElement("code", { className: "language-text" }, document.source)),
      );
    }
  ...
  private assertSafe(source: string): void {
    ...
    if (unsafeMdxPattern.test(expressionSurface)) {
      throw new Error("Garments MDX allows Markdown and component markup, but not executable expressions.");
    }
  }

  private replaceWikiLinks(source: string): string {
    return source.replace(/\[\[([^\]|#]+)(?:#([^\]|]+))?(?:\|([^\]]+))?\]\]/g, (_match, target, _heading, label) => {
