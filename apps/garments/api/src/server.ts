...
const profile = readApplicationDeployableProfile({ applicationId: "garments", availableProviderIds: ["platform.core", provider.manifest.id] });
...
await app.register(cors, { origin: process.env.GARMENTS_WEB_ORIGIN, methods: ["GET", "HEAD", "OPTIONS", "POST", "PUT", "DELETE"], allowedHeaders: ["Authorization", "Content-Type", "X-Codexsun-Browser-Session", "X-Codexsun-Auto-Login-Desk"] });
await app.register(swagger, { openapi: { info: { title: "Garments API", version: "1.0.0" }, openapi: "3.0.3" }, transform: jsonSchemaTransform });
...
app.post("/api/v1/garments/auth/login", { schema: { body: identityLoginSchema, response: { 200: identityLoginResponseSchema, 400: identityErrorResponseSchema, 401: identityErrorResponseSchema, 429: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/garments/auth/:portal/login", { schema: { params: z.object({ portal: z.enum(["admin", "super-admin"]) }), body: identityLoginSchema, response: { 200: identityLoginResponseSchema, 400: identityErrorResponseSchema, 401: identityErrorResponseSchema, 429: identityErrorResponseSchema } } }, async (request, reply) => {
  const credentials = identityLoginSchema.safeParse(request.body);
...
app.post("/api/v1/garments/auth/password-reset/request", { schema: { body: identityPasswordResetRequestSchema, response: { 202: identityPasswordResetAcceptedSchema } } }, async (request, reply) => {
...
app.post("/api/v1/garments/auth/password-reset/confirm", { schema: { body: identityPasswordResetConfirmationSchema, response: { 204: z.null(), 400: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/garments/auth/development-login", async (request, reply) => {
  const browserSessionId = identityBrowserSessionIdSchema.safeParse(request.headers["x-codexsun-browser-session"]);
...
app.post("/api/v1/garments/auth/logout", async (request, reply) => reply.code(identity.logout(request.headers.authorization, request.headers["x-codexsun-browser-session"]) ? 204 : 401).send());
registerIdentityManagementRoutes({ app, identity, prefix: "/api/v1/garments" });
app.get("/api/v1/garments/health", { schema: { response: { 200: z.object({ status: z.literal("ok"), providers: z.array(z.string()) }) }, tags: ["System"] } }, async () => ({ status: "ok" as const, providers: [...runtime.enabledProviderIds] }));
app.addHook("onClose", () => { identity.close(); runtime.stop(); });
await app.listen({ host: config.host, port: config.port });

function isPublicPath(url: string): boolean {
  const path = new URL(url, "http://localhost").pathname;
  return path === "/api/v1/garments/auth/login"
    || path === "/api/v1/garments/auth/admin/login"
    || path === "/api/v1/garments/auth/super-admin/login"
    || path === "/api/v1/garments/auth/development-login"
    || path === "/api/v1/garments/auth/password-reset/request"
    || path === "/api/v1/garments/auth/password-reset/confirm"
    || path === "/api/v1/garments/health"
