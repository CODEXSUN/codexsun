...
function LeadPage({ request }: { request: typeof fetch }) {
  const queryClient = useQueryClient();
  const leads = useQuery({ queryKey: ["crm", "/api/v1/crm/leads"], queryFn: () => readJson<{ items: Record<string, unknown>[] }>(request, "/api/v1/crm/leads") });
  const [selectedLead, setSelectedLead] = useState<string>();
  const [score, setScore] = useState("50");
  const [subject, setSubject] = useState("");
  const mutation = useMutation({
    mutationFn: async (action: { id: string; kind: "qualify" | "convert" }) => {
      const path = action.kind === "qualify" ? `/api/v1/crm/leads/${action.id}/qualify` : `/api/v1/crm/leads/${action.id}/convert`;
      const body = action.kind === "qualify" ? { note: "Qualified from CRM workspace", score: Number(score) } : { subject: subject.trim() || "Customer service enquiry" };
      return postJson(request, path, body, action.kind === "qualify" ? "PATCH" : "POST");
    },
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["crm", "/api/v1/crm/leads"] }); void queryClient.invalidateQueries({ queryKey: ["crm", "overview"] }); setSelectedLead(undefined); },
  });
  return <section className="space-y-6 p-6"><header className="border-b border-border pb-5"><p className="text-sm font-medium text-muted-foreground">CRM module</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">Leads</h1><p className="mt-2 text-sm text-muted-foreground">Capture, qualify, and convert new customer opportunities.</p></header><div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]"><div className="border border-border bg-card"><div className="border-b border-border px-5 py-4"><h2 className="font-semibold">Lead queue</h2></div>{leads.isPending ? <p className="p-5 text-sm text-muted-foreground">Loading...</p> : leads.isError ? <p className="p-5 text-sm text-destructive">Could not load leads.</p> : leads.data.items.length === 0 ? <p className="p-5 text-sm text-muted-foreground">No leads yet.</p> : <div className="divide-y divide-border">{leads.data.items.map((item, index) => { const id = String(item.id ?? index); const status = String(item.status ?? "new"); return <div className="flex items-center justify-between gap-4 px-5 py-4" key={id}><div><p className="text-sm font-medium">{String(item.name ?? "Lead")}</p><p className="mt-1 text-xs capitalize text-muted-foreground">{status.replace("_", " ")} · score {String(item.score ?? 0)}</p></div><div className="flex gap-2">{status === "new" ? <button className="rounded-md border border-border px-3 py-1.5 text-xs font-medium" onClick={() => { setSelectedLead(id); mutation.mutate({ id, kind: "qualify" }); }} type="button">Qualify</button> : null}{status === "qualified" ? <button className="rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground" onClick={() => { setSelectedLead(id); mutation.mutate({ id, kind: "convert" }); }} type="button">Convert</button> : null}</div></div>; })}</div>}</div><div className="border border-border bg-card p-5"><h2 className="font-semibold">Lead actions</h2><p className="mt-1 text-xs text-muted-foreground">Set conversion details before moving a lead to an enquiry.</p><label className="mt-5 block text-xs font-medium" htmlFor="lead-score">Qualification score</label><input className="mt-2 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" id="lead-score" max="100" min="0" onChange={(event) => setScore(event.target.value)} type="number" value={score} /><label className="mt-4 block text-xs font-medium" htmlFor="lead-subject">Enquiry subject</label><input className="mt-2 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" id="lead-subject" onChange={(event) => setSubject(event.target.value)} placeholder="Service request" value={subject} /><p className="mt-4 text-xs text-muted-foreground">{mutation.isPending && selectedLead ? "Saving lead workflow..." : "Select a lead to qualify or convert."}</p></div></div></section>;
}
...
function moduleConfig(view: WorkspaceView): ModuleConfig {
  ...
  if (view === "customers") return { description: "Keep accounts, contacts, addresses, and consent in one customer view.", field: "name", label: "Customers", path: "/api/v1/crm/customers", placeholder: "Customer or company name", singular: "customer" };
...
async function postJson<T>(request: typeof fetch, path: string, body: Record<string, unknown>, method: "POST" | "PATCH" = "POST"): Promise<T> { const response = await request(path, { body: JSON.stringify(body), headers: { "Content-Type": "application/json" }, method }); if (!response.ok) throw new Error(`Request failed: ${response.status}`); return response.json() as Promise<T>; }
