...
await app.register(cors, { origin: process.env.CRM_WEB_ORIGIN, methods: ["GET", "HEAD", "OPTIONS", "POST", "PUT"], allowedHeaders: ["Authorization", "Content-Type", "X-Codexsun-Browser-Session"] });
...
app.post("/api/v1/crm/auth/login", { schema: { body: identityLoginSchema, response: { 200: identityLoginResponseSchema, 400: identityErrorResponseSchema, 401: identityErrorResponseSchema, 429: identityErrorResponseSchema } } }, async (request, reply) => {
  ...
  let session;
  try {
    session = await identity.login(credentials.data.identifier, credentials.data.password, browserSessionId.data, "user");
  } catch (error) {
...
app.post("/api/v1/crm/auth/:portal/login", { schema: { params: z.object({ portal: z.enum(["admin", "super-admin"]) }), body: identityLoginSchema, response: { 200: identityLoginResponseSchema, 400: identityErrorResponseSchema, 401: identityErrorResponseSchema, 429: identityErrorResponseSchema } } }, async (request, reply) => {
  const credentials = identityLoginSchema.safeParse(request.body);
...
app.post("/api/v1/crm/auth/password-reset/confirm", { schema: { body: identityPasswordResetConfirmationSchema, response: { 204: z.null(), 400: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/crm/auth/logout", async (request, reply) => {
...
function isPublicPath(url: string): boolean {
  ...
  return path === "/api/v1/crm/auth/login"
    || path === "/api/v1/crm/auth/admin/login"
    || path === "/api/v1/crm/auth/super-admin/login"
    || path === "/api/v1/crm/auth/development-login"
    || path === "/api/v1/crm/auth/password-reset/request"
    || path === "/api/v1/crm/auth/password-reset/confirm"
    || path === "/api/v1/crm/health"
