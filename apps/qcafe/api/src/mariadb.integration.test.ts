...
const connectionUrl = process.env.QCAFE_MARIADB_INTEGRATION_URL;

test("runs qcafe migrations against an explicit MariaDB test database", { skip: !connectionUrl }, async () => {
  ...
  try {
    ...
    await runner.run({ moduleId: "qcafe.integration", migrations: [], seeders: [] });
  } finally {
