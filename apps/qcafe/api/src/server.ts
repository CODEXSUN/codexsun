...
const profile = readApplicationDeployableProfile({ applicationId: "qcafe", availableProviderIds: ["platform.core", provider.manifest.id] });
...
await app.register(cors, { origin: process.env.QCAFE_WEB_ORIGIN, methods: ["GET", "HEAD", "OPTIONS"], allowedHeaders: ["Authorization", "Content-Type"] });
await app.register(swagger, { openapi: { info: { title: "Q Cafe API", version: "1.0.0" }, openapi: "3.0.3" }, transform: jsonSchemaTransform });
...
app.get("/api/v1/qcafe/health", { schema: { response: { 200: z.object({ status: z.literal("ok"), providers: z.array(z.string()) }) }, tags: ["System"] } }, async () => ({ status: "ok" as const, providers: [...runtime.enabledProviderIds] }));
