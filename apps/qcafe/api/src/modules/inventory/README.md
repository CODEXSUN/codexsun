...
Purchase order and receipt lines carry `unit_price_minor` (migration `008`). A receipt line without an explicit price inherits its purchase order line price. Stock valuation multiplies on-hand milli-quantities by the latest receipt unit price; valuation never posts ledger movements and never touches the accounting module, which stays free of fabricated cost postings until unit costs are approved.
