...
test("manages database status, cloud sync policy, and safe connector metadata", async () => {
  const configuration = { localDatabasePath: ":memory:", mode: "local" as const, syncCloudUrl: "https://cloud.example/qcafe/sync" };
  ...
  assert.equal(database.lifecycleRecords, 7);
  ...
  let connectors = await service.createConnector({ code: "SWIGGY", kind: "marketplace", name: "Swiggy orders" }, context);
  await assert.rejects(service.setConnectorEnabled(connectors.connectors[0]!.id, true, context), SettingsConflictError);
  connectors = await service.createConnector({ code: "ZOMATO", endpointLabel: "Production intake", kind: "marketplace", name: "Zomato orders", secretReference: "QCAFE_ZOMATO_SECRET" }, context);
