...
export function StorefrontHeroSliderDesktop({ landing }: { landing: StorefrontLandingResponse }) {
  ...
  return (
    <section
      className="relative isolate hidden w-full max-w-full min-w-0 overflow-hidden rounded-[2.6rem] border border-[#e1d3c4] shadow-[0_35px_95px_-46px_rgba(52,26,15,0.42)] lg:block lg:min-h-[540px] xl:min-h-[580px]"
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-y-10 left-[50%] w-px -translate-x-1/2" style={{ background: `linear-gradient(180deg, transparent 0%, ${model.sliderStyles.innerFrameBorderColor} 18%, ${model.sliderStyles.innerFrameBorderColor} 82%, transparent 100%)`, opacity: 0.55 }} />
        ...
        <div className="grid h-full w-full grid-cols-[minmax(0,1fr)_minmax(460px,1.06fr)] items-center gap-10 xl:gap-14 2xl:grid-cols-[minmax(0,1.02fr)_minmax(540px,1.08fr)]">
          <div className="flex h-full flex-col">
            ...
            <AnimatePresence mode="wait" initial={false} custom={model.direction}>
              <motion.div key={`${model.activeItem.id}:content`} custom={model.direction} initial={{ opacity: 0, x: model.direction > 0 ? 24 : -24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: model.direction > 0 ? -16 : 16 }} transition={smoothContentTransition} className="my-auto -translate-y-2 space-y-7 xl:space-y-8">
                <motion.div initial="initial" animate="animate" exit="exit" transition={staggeredContentTransition}>
                  ...
                  <motion.p variants={staggeredContentItem} className="line-clamp-2 max-w-2xl overflow-hidden pt-5 text-base leading-8 sm:text-lg xl:max-w-3xl" style={{ color: model.sliderStyles.mutedTextColor }}>
                    {model.activeSummary}
                  </motion.p>
          ...
          <div className="relative flex h-full items-center justify-center xl:-my-6">
            <div className="relative flex h-full w-full items-center justify-center">
              <div className="pointer-events-none absolute inset-x-8 top-1/2 h-[72%] -translate-y-1/2 rounded-[2.4rem] border" style={{ borderColor: model.sliderStyles.outerFrameBorderColor, background: "linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.01))", opacity: 0.55 }} />
              <AnimatePresence mode="wait" initial={false} custom={model.direction}>
                <motion.div key={`${model.activeItem.id}:image`} custom={model.direction} initial={{ opacity: 0, x: model.direction > 0 ? 24 : -24, scale: 0.992 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: model.direction > 0 ? -18 : 18, scale: 0.992 }} transition={{ ...smoothImageTransition, delay: 0.06 }} className="relative z-[1] h-[500px] w-[560px] max-w-[560px] rounded-[2rem] p-3 backdrop-blur-[24px] xl:h-[540px] xl:w-[620px] xl:max-w-[620px]" style={{ background: model.sliderStyles.frameBackground, boxShadow: `inset 0 1px 0 rgba(255,255,255,0.34), 0 0 0 1px ${model.sliderStyles.outerFrameBorderColor}, 0 28px 72px -42px rgba(24,12,7,0.34), 0 12px 38px -20px rgba(255,255,255,0.18)` }}>
                  <div className="flex h-full overflow-hidden rounded-[1.7rem] border p-px" style={{ background: model.sliderStyles.imagePanelBackground, borderColor: model.sliderStyles.innerFrameBorderColor }}>
                    <div className="h-full w-full overflow-hidden rounded-[1.58rem]" style={{ background: model.sliderStyles.imagePanelBackground }}>
                      <StorefrontImage imageUrl={model.activeItem.primaryImageUrl ?? landing.settings.hero.heroImageUrl} fallbackLabel={model.activeItem.name} alt={model.activeItem.name} width={1240} height={1500} className="h-full w-full object-cover object-center" loading="eager" decoding="async" fetchPriority="high" />
                    </div>
                  </div>
                </motion.div>
              </AnimatePresence>
