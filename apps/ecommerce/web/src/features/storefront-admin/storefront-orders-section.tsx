  ...
  const tabs: AnimatedContentTab[] = [
    {
      ...
      content: <OrderQueueList items={getFilteredItems("all")} />,
    },
    {
      value: "payment_attention",
      label: `Action required (${report?.summary.actionRequiredCount ?? 0})`,
      content: <OrderQueueList items={getFilteredItems("payment_attention")} />,
    },
    {
      value: "fulfilment",
      label: `Fulfilment (${report?.summary.fulfilmentQueueCount ?? 0})`,
      content: <OrderQueueList items={getFilteredItems("fulfilment")} />,
    },
    {
      value: "shipment",
      label: `Shipment (${report?.summary.shipmentQueueCount ?? 0})`,
      content: <OrderQueueList items={getFilteredItems("shipment")} />,
    },
    {
      value: "pickup",
      label: `Pickup (${report?.summary.pickupQueueCount ?? 0})`,
      content: <OrderQueueList items={getFilteredItems("pickup")} />,
    },
    {
      value: "completed",
      label: `Completed (${report?.summary.completedCount ?? 0})`,
      content: <OrderQueueList items={getFilteredItems("completed")} />,
    },
    {
      value: "closed",
      label: `Closed (${report?.summary.closedCount ?? 0})`,
      content: <OrderQueueList items={getFilteredItems("closed")} />,
    },
  ]
