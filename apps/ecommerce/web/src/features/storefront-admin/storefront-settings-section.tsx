...
import {
  ...
  validateStorefrontSettingsDesigner,
...
export function StorefrontSettingsSection() {
  ...
  const validationIssues = draft ? validateStorefrontSettingsDesigner(draft) : []
  const hasValidationIssues = validationIssues.length > 0

  useGlobalLoading(isLoading || isSaving || isPublishing || isRollingBack)
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button type="button" className="rounded-full" disabled={!canEditStorefrontDesigner || isSaving || hasValidationIssues} onClick={async () => {
          if (!accessToken) {
            setError("Admin session is required.")
            return
          }
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              setSaveMessage("Draft published to the live storefront.")
              showRecordToast({
                entity: "Storefront Settings",
                action: "published",
                recordName: "Storefront",
                recordId: workflowState.liveSettings.id,
              })
            } catch (publishError) {
              const message =
                publishError instanceof Error
                  ? publishError.message
                  : "Failed to publish storefront settings."
              setError(message)
              showAppToast({
                variant: "error",
                title: "Storefront publish failed.",
                description: message,
              })
            } finally {
        ...
        <Button
          ...
          variant="outline"
          className="rounded-full"
          disabled={!canEditStorefrontDesigner || !canApproveStorefrontDesigner || isRollingBack || !workflow?.revisions.length}
          onClick={async () => {
            if (!accessToken) {
              setError("Admin session is required.")
              return
            ...
            setError(null)
            try {
              const workflowState = await storefrontApi.rollbackStorefrontSettings(
                accessToken,
              ...
              invalidateStorefrontShellData()
              await refreshShellData().catch(() => null)
              setSaveMessage("Live storefront rolled back to the latest saved revision.")
              showRecordToast({
                entity: "Storefront Settings",
                action: "rolled back",
                recordName: "Storefront",
              ...
              const message =
                rollbackError instanceof Error
                  ...
                  : "Failed to rollback storefront settings."
              setError(message)
              showAppToast({
                variant: "error",
                title: "Storefront rollback failed.",
                description: message,
              })
            } finally {
        ...
        >
          {isRollingBack ? "Rolling back..." : "Rollback live"}
        </Button>
