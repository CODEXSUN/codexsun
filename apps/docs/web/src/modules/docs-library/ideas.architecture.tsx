...
const repositoryComparison = [
  ...
  ["Contributor guidance", "AGENTS, Assist standards, module READMEs, and development records guide each change."],
  ["Integration and release", "npm workspaces and the runtime catalog build selected applications together."],
] as const;
    ...
    <div className="mx-auto w-4/5 max-w-[120rem] px-6 py-10 lg:px-10">
      <TopologyRegion className="max-w-4xl" id="11.6" topology={topology}>
        ...
        <p className="mt-4 max-w-3xl text-base leading-7 text-muted-foreground">
          A multi-developer repository stays understandable when every change has an owner, contract, boundary, and
          proof. These rules guide a team without centralizing product work.
        </p>
      </TopologyRegion>
      ...
      <TopologyRegion className="mt-10" id="11.7" topology={topology}>
        ...
        <ol className="mt-5 grid gap-3 lg:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr_auto_1fr] lg:items-center">
          <ChangeStep detail="Describe the user or product need." icon={ClipboardCheckIcon} title="Need" />
          <FlowArrow />
          <ChangeStep detail="Choose the application or package owner." icon={UsersRoundIcon} title="Owner" />
          <FlowArrow />
          <ChangeStep detail="Define the public contract and failure case." icon={BookOpenCheckIcon} title="Contract" />
          <FlowArrow />
          <ChangeStep detail="Implement inside the owning boundary." icon={GitPullRequestArrowIcon} title="Change" />
          <FlowArrow />
          <ChangeStep detail="Run checks, review, and record the result." icon={ShieldCheckIcon} title="Proof" />
        </ol>
      </TopologyRegion>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {architectureStandards.map((standard) => (
            <Card key={standard.title} className="h-full">
              ...
              <CardContent className="leading-6 text-muted-foreground">{standard.detail}</CardContent>
            </Card>
          ))}
        </div>
      </TopologyRegion>
          ...
          <Table className="min-w-[46rem] text-left">
            <TableHeader className="bg-muted/60 text-foreground">
              <TableRow>
                ...
                <TableHead className="px-4 py-3 font-semibold">Current repository pattern</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
      ...
      <TopologyRegion className="mt-12 border-t pt-6" id="11.10" topology={topology}>
        ...
        <p className="mt-3 max-w-4xl leading-7 text-muted-foreground">
          A developer first reads the owner guidance. The developer then changes one boundary, validates that boundary,
          and records the decision. Shared code grows only after a real application proves the need.
        </p>
      </TopologyRegion>
    </div>
  );
...
function ChangeStep({ detail, icon: Icon, title }: { detail: string; icon: typeof ClipboardCheckIcon; title: string }) {
  return (
