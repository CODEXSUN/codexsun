...
import { getAdjacentDocuments, getDocsIndexGroups, getDocsNavigationNodes } from "./docs-library.index";
import { DocsIndexPage } from "./docs-library.index-page";
import type { DocsState } from "./docs-library.types";
import { getBacklinks, getDocumentHeadings, getRelatedDocuments, matchesDocument } from "./docs-library.utils";
...
function DocsLibraryView({ library, onEdit }: { library: ReturnType<typeof useDocsLibrary>; onEdit: () => void }) {
        ...
        >
          ...
          {!error && activeDocument ? (
            <div className="docs-reading-layout">
              <TopologyRegion as="article" className="docs-content min-w-0 max-w-none" id="10.2.1" topology={topology}>
          ...
          {!error && !activeDocument ? <DocsIndexPage documents={documents} onSelect={selectDocument} /> : null}
        </div>
...
function DocumentNavigation({ documents, slug }: { documents: DocsState["documents"]; slug: string }) {
  const topology = useMdiTopology();
...
function DocumentOutline({ source }: { source: string }) {
  ...
  return (
    <TopologyRegion as="aside" className="docs-outline" id="10.2.2" topology={topology} aria-label="On this page">
...
function UnavailablePage({ error }: { error: string }) {
  const topology = useMdiTopology();
  return (
    <TopologyRegion as="article" className="docs-content max-w-[75ch]" id="10.4" topology={topology}>
