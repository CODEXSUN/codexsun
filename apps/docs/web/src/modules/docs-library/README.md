...
- Editor: the header Edit action opens a Docs-owned drill-down upsert page with an authoritative Markdown mode, explicit Save and Back controls, and a stale-source conflict message.
...
- Ownership map: package and application cards share the centered icon-over-title card treatment. The application row keeps every current app on one wide-screen row and wraps progressively on narrower screens.
...
- Settings: `VITE_DOCS_API_URL` selects the Docs API. Define it from the root `.env`; no application fallback is provided. The shared Settings entry opens the read-only Documentation health scan. It reports missing README files and unorganized Markdown, MDX, and text sources without moving or changing them.
