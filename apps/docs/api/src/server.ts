...
const profile = readApplicationDeployableProfile({ applicationId: "docs", availableProviderIds: ["platform.core", catalogProvider.manifest.id] });
const runtime = createPlatformRuntime(profile, [catalogProvider, ...(await loadEnabledAddonProviders(profile))]);
