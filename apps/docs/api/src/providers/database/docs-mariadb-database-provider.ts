...
export type DocsMariaDbDatabase<TDatabase = Record<string, never>> = Kysely<TDatabase>;

export class DocsMariaDbDatabaseProvider<TDatabase = Record<string, never>> implements DatabaseProvider<
  DocsMariaDbDatabase<TDatabase>
> {
  readonly client: DocsMariaDbDatabase<TDatabase>;

  constructor(environment: DocsEnvironment) {
