...
const connectionUrl = process.env.AUDITOR_MARIADB_INTEGRATION_URL;

test("runs auditor migrations against an explicit MariaDB test database", { skip: !connectionUrl }, async () => {
  ...
  try {
    ...
    await runner.run({ moduleId: "auditor.integration", migrations: [], seeders: [] });
  } finally {
