...
await app.register(cors, { origin: process.env.HIMSX_WEB_ORIGIN, methods: ["GET", "HEAD", "OPTIONS", "POST", "PUT", "DELETE"], allowedHeaders: ["Authorization", "Content-Type", "X-Codexsun-Browser-Session", "X-Codexsun-Auto-Login-Desk"] });
...
app.post("/api/v1/himsx/auth/login", { schema: { body: identityLoginSchema, response: { 200: identityLoginResponseSchema, 400: identityErrorResponseSchema, 401: identityErrorResponseSchema, 429: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/himsx/auth/:portal/login", { schema: { params: z.object({ portal: z.enum(["admin", "super-admin"]) }), body: identityLoginSchema, response: { 200: identityLoginResponseSchema, 400: identityErrorResponseSchema, 401: identityErrorResponseSchema, 429: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/himsx/auth/password-reset/request", { schema: { body: identityPasswordResetRequestSchema, response: { 202: identityPasswordResetAcceptedSchema } } }, async (request, reply) => {
...
app.post("/api/v1/himsx/auth/password-reset/confirm", { schema: { body: identityPasswordResetConfirmationSchema, response: { 204: z.null(), 400: identityErrorResponseSchema } } }, async (request, reply) => {
...
app.post("/api/v1/himsx/auth/development-login", async (request, reply) => {
  const browserSessionId = identityBrowserSessionIdSchema.safeParse(request.headers["x-codexsun-browser-session"]);
...
app.post("/api/v1/himsx/auth/logout", async (request, reply) => reply.code(identity.logout(request.headers.authorization, request.headers["x-codexsun-browser-session"]) ? 204 : 401).send());
registerIdentityManagementRoutes({ app, identity, prefix: "/api/v1/himsx" });
...
function isPublicPath(url: string): boolean {
  ...
  return path === "/api/v1/himsx/auth/login"
    || path === "/api/v1/himsx/auth/admin/login"
    || path === "/api/v1/himsx/auth/super-admin/login"
    || path === "/api/v1/himsx/auth/development-login"
    || path === "/api/v1/himsx/auth/password-reset/request"
    || path === "/api/v1/himsx/auth/password-reset/confirm"
    || path === "/api/v1/himsx/health"
