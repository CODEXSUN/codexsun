...
export const billingPaymentItemVouchersMigration = defineDatabaseMigration({
  id: "billing:vouchers:20-payment-item-vouchers",
  appId: "billing",
  moduleKey: "payment-item-vouchers",
  name: "Create billing payment item voucher detail table",
  order: 200,
  up: async ({ database }) => {
    await asQueryDatabase(database).schema
      .createTable(billingTableNames.paymentItemVouchers)
      .ifNotExists()
      ...
      .addColumn("tds_deducted", "real", (col) => col.notNull().defaultTo(0))
