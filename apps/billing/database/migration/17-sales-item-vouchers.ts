...
export const billingSalesItemVouchersMigration = defineDatabaseMigration({
  id: "billing:vouchers:17-sales-item-vouchers",
  appId: "billing",
  moduleKey: "sales-item-vouchers",
  name: "Create billing sales item voucher detail table",
  order: 170,
  up: async ({ database }) => {
    await asQueryDatabase(database).schema
      .createTable(billingTableNames.salesItemVouchers)
      .ifNotExists()
