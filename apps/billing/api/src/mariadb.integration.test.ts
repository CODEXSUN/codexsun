...
const connectionUrl = process.env.BILLING_MARIADB_INTEGRATION_URL;

test("runs billing migrations against an explicit MariaDB test database", { skip: !connectionUrl }, async () => {
  ...
  try {
    ...
    await runner.run({ moduleId: "billing.integration", migrations: [], seeders: [] });
  } finally {
