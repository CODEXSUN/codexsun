...
export const coreMailboxSeeder = defineDatabaseSeeder({
  ...
  run: async ({ database }) => {
    ...
    await queryDatabase.deleteFrom(coreTableNames.mailboxMessageRecipients).execute()
    await queryDatabase.deleteFrom(coreTableNames.mailboxMessages).execute()
    await queryDatabase.deleteFrom(coreTableNames.mailboxTemplates).execute()
