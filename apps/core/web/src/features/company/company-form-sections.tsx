...
import type { LocalOption } from "./company-form-state"

export function CompanyFormMessage({ children }: { children: ReactNode }) {
  return (
...
export function CompanyFormSectionCard({
  children,
  description,
  onAdd,
  title,
...
export function CompanyCollectionRow({
  children,
  onRemove,
}: {
  children: ReactNode
...
export function CompanyStatusField({
  checked,
  id,
  onCheckedChange,
}: {
      ...
      >
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {checked
              ? "This company is available in the workspace."
              : "This company is hidden as inactive."}
          </p>
        </div>
...
export function CompanyTextField(props: React.ComponentProps<typeof Input>) {
  return <Input {...props} />
}
