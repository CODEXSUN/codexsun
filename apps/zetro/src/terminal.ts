...
function printUsage() {
  ...
  console.info("  status                       Show current lock, data source, and catalog counts.")
...
function printSummary(data: ZetroTerminalData) {
  ...
  console.info(`Approval-gated playbooks: ${data.summary.approvalRequiredPlaybooks}`)
...
function runDoctor(data: ZetroTerminalData) {
  ...
  const playbookIdDuplicates = getDuplicateValues(data.playbooks.map((playbook) => playbook.id))
  const guardrailIdDuplicates = getDuplicateValues(data.guardrails.map((guardrail) => guardrail.id))
...
function printAssist(data: ZetroTerminalData) {
  ...
  console.info(`- command execution: ${data.settings.runtimeLock.commandExecution}`)
  ...
  console.info(`- autonomous loop: ${data.settings.runtimeLock.autonomousLoop}`)
}
...
function printPlaybook(data: ZetroTerminalData, playbookId: string | undefined) {
  if (!playbookId) {
