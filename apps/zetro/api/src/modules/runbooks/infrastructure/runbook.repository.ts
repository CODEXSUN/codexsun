...
type RunbookRow = { id: string; title: string; prompt: string; repository_path: string; module_path: string; interval_minutes: number; schedule_mode: 'one-time' | 'repeating'; enabled: number; next_run_at: number; last_run_at: number | null; created_at: number; updated_at: number }
...
export class SqliteRunbookRepository implements RunbookStore {
  ...
  list() { return (this.database.prepare('SELECT * FROM runbooks ORDER BY created_at DESC').all() as unknown as RunbookRow[]).map(mapRunbook) }
