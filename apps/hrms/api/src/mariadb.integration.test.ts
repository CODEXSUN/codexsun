...
const connectionUrl = process.env.HRMS_MARIADB_INTEGRATION_URL;

test("runs hrms migrations against an explicit MariaDB test database", { skip: !connectionUrl }, async () => {
  ...
  try {
    ...
    await runner.run({ moduleId: "hrms.integration", migrations: [], seeders: [] });
  } finally {
