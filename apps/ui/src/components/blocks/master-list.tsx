...
export type MasterListActiveFilter = {
  key: string
  label: string
  value: string
}

export type MasterListFilterOption = {
  key: string
  label: string
  isActive?: boolean
  onSelect: () => void
...
export type MasterListColumn<TData> = {
  id: string
  header: string
...
export function MasterList<TData>({
  header,
  search,
  filters,
  table,
  ...
  const visibleColumns = table.columns.filter((column) =>
    (visibleColumnIds.length > 0 ? visibleColumnIds : defaultVisibleColumnIds).includes(column.id)
  )
  const fallbackColumns = visibleColumns.length > 0 ? visibleColumns : table.columns.slice(0, 1)
  const sortColumn = sortState
    ? table.columns.find((column) => column.id === sortState.columnId)
    : undefined

  const sortedData =
    sortColumn && sortColumn.sortable
      ...
      : table.data
  const pageRowKeys = sortedData.map((row, index) =>
    table.rowKey ? table.rowKey(row, index) : index
  )
  const allVisibleRowsSelected =
    rowSelection && pageRowKeys.length > 0
      ? pageRowKeys.every((rowId) => rowSelection.selectedRowIds.includes(rowId))
      : false

  const derivedActiveFilters: MasterListActiveFilter[] = search?.value.trim()
    ? [{ key: "__search__", label: "Search", value: search.value.trim() }]
    : []
  const activeFilters = [...derivedActiveFilters, ...(filters?.activeFilters ?? [])]

  const pageSizeOptions =
    pagination?.pageSizeOptions?.length ? pagination.pageSizeOptions : DEFAULT_PAGE_SIZE_OPTIONS
  ...
  const showSearchSection = Boolean(
    search || (filters?.options && filters.options.length > 0) || table.columns.length > 1
  )
  const showActiveFilters = activeFilters.length > 0
  const hasRows = sortedData.length > 0
  const effectiveColumnCount = fallbackColumns.length + (rowSelection ? 1 : 0)
  ...
  const toggleAllVisibleRows = (checked: boolean) => {
    ...
    if (checked) {
      const nextRowIds = Array.from(
        new Set([...rowSelection.selectedRowIds, ...pageRowKeys])
      )
      rowSelection.onSelectedRowIdsChange(nextRowIds)
      return
    }

    rowSelection.onSelectedRowIdsChange(
      rowSelection.selectedRowIds.filter((rowId) => !pageRowKeys.includes(rowId))
    )
  }

  const handleSort = (column: MasterListColumn<TData>) => {
    if (!column.sortable) {
      return
    }
        ...
        <section className="rounded-md border bg-card p-4 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex flex-wrap gap-2">
              {activeFilters.map((filter) => (
                <Badge
                  key={filter.key}
                  variant="secondary"
                  className="h-auto gap-2 rounded-md px-3 py-1 text-xs"
                >
                  <span>
            ...
            <Button
              type="button"
              variant="ghost"
              className="h-8 justify-start rounded-md px-0 text-sm sm:px-3"
              onClick={handleClearAllFilters}
            >
              Clear all filters
            </Button>
        ...
        <Table className="min-w-full">
          <TableHeader className="bg-muted">
            <TableRow className="sticky top-0 z-10 hover:bg-muted">
              {rowSelection ? (
                <TableHead className={cn("w-12 px-4 py-3", getStickyClass("left", "header"))}>
                  <div className="flex items-center justify-center">
                    <Checkbox
                      checked={allVisibleRowsSelected}
                      onCheckedChange={(checked) => toggleAllVisibleRows(Boolean(checked))}
                      aria-label={rowSelection.selectionLabel ?? "Select all rows"}
                    />
                  </div>
                </TableHead>
              ) : null}
              {fallbackColumns.map((column) => {
                ...
                return (
                  <TableHead
                    ...
                    className={cn(
                      "px-4 py-3",
                      getStickyClass(column.sticky, "header"),
          ...
          <TableBody>
            {table.loading && !hasRows ? (
              ...
              sortedData.map((row, index) => {
                const rowId = table.rowKey ? table.rowKey(row, index) : index
                const isSelected = rowSelection?.selectedRowIds.includes(rowId) ?? false
                ...
                >
                  {rowSelection ? (
                    <TableCell
                      className={cn("w-12 px-4 py-3 align-top", getStickyClass("left"))}
                    >
                      <div className="flex items-center justify-center pt-0.5">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            rowSelection.onSelectedRowIdsChange(
                              checked
                                ? Array.from(new Set([...rowSelection.selectedRowIds, rowId]))
                                : rowSelection.selectedRowIds.filter((currentId) => currentId !== rowId)
                            )
                          }}
                ...
                <PaginationContent className="flex-nowrap items-center justify-end gap-1">
                  ...
                  <PaginationItem>
                    <PaginationNext
                      href="#"
                      className={cn(
                        pagination.currentPage >= totalPages && "pointer-events-none opacity-50"
                      )}
                      onClick={(event) => {
                        event.preventDefault()
...
export const CommonList = MasterList
