...
const connectionUrl = process.env.LMS_MARIADB_INTEGRATION_URL;

test("runs lms migrations against an explicit MariaDB test database", { skip: !connectionUrl }, async () => {
  ...
  try {
    ...
    await runner.run({ moduleId: "lms.integration", migrations: [], seeders: [] });
  } finally {
