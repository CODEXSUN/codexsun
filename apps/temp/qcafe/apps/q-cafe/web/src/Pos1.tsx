...
import { directPrintReceipt } from './rawReceipt';
import {
  Pos1HeaderSection,
  Pos1ProductSection,
  Pos1BillingSection,
...
export function Pos1({ data, busy, mutate, topology }: Props) {
  ...
  async function handleSendToKitchen() {
    ...
    if (sent) {
      setShowPaymentCollector(false);
      if (cafeSettings.autoPrintBill && !cafeSettings.directPrint) {
  ...
  // Section 6: Save the POS bill. Cashiers can save and print immediately (defaulting to Cash if unset).
  async function handleSaveAndPrintBill() {
    ...
    await handleConfirmOrder(payment);
  }

  async function handleConfirmOrder(paymentOverride?: PaymentRecord | null) {
    if (!lines.length || busy) return;
    ...
    if (payment) {
      ...
      // A settled bill uses direct printing without preview when enabled, falling back to print dialog on error or interactive port.
      if (cafeSettings.directPrint) {
        try {
          ...
          setPrintNotice('Receipt sent directly to printer.');
        } catch (error) {
          console.warn('Direct print fallback to standard print:', error);
          const errorMsg = error instanceof Error ? error.message : String(error);
          setPrintNotice(
            errorMsg.includes('INTERACTIVE_PRINTER')
              ? 'Opening print preview dialog...'
              : `Direct print unavailable (${errorMsg}). Opening print dialog...`
          );
  ...
  useEffect(() => {
    const handleKeyDown = (e: globalThis.KeyboardEvent) => {
      ...
      // F6 or F7: open the current-order receipt / settlement drawer
  ...
  }, [lines.length, busy, activeTabId, tabs, tableName, cafeSettings, showPaymentCollector, showPreviousBillsDrawer, selectedPreviousBillId, gstApplied, activeTab.chair, activeTab.payment, total, orderMode, data.pos, data.restaurant_tables]);
  ...
  return (
    ...
    >
      ...
      {printNotice && (
          <div className={`mx-4 mt-3 rounded-lg border px-3 py-2 text-sm ${printNotice.startsWith('Receipt sent directly') ? 'border-emerald-500/70 bg-emerald-500/10 text-emerald-700' : 'border-amber-500/70 bg-amber-500/10 text-amber-800'}`}>
          {printNotice}
        </div>
      )}
