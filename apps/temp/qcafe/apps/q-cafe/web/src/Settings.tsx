...
export function Settings({ data, mutate, topology }: Props) {
  ...
  async function installPrinterService() {
    ...
    try {
      ...
      setPrinterServiceNotice('The Q Cafe Node installer has started. Approve the Windows administrator prompt, then use Printer & Receipts again to refresh the status.');
      window.setTimeout(() => void loadPrinterServiceStatus(), 3_000);
    } catch (error) {
      setPrinterServiceNotice(error instanceof Error ? error.message : 'Q Cafe could not install the Raw Print Service.');
    } finally {
  ...
  async function selectTodaySpecialPrefix(prefix: string) {
    await mutate('master-setting', { key: 'today_special_selected_prefix', value: prefix });
  }
                ...
                <div className="space-y-5">
                  ...
                  {printerServiceStatus && (
                    <div className={`flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-start sm:justify-between ${printerServiceStatus.connected ? 'border-emerald-500/70 bg-emerald-500/10' : 'border-border bg-muted/20'}`}>
                      ...
                      <div className="min-w-0 space-y-1">
                        <p className="text-sm font-semibold text-foreground">{printerServiceStatus.connected ? 'Raw Print Service connected' : 'Raw Print Service needs attention'}</p>
                  ...
                  <label className="grid gap-1.5 text-sm font-medium">
                    ...
                    <select
                      ...
                      onChange={(event) => handlePrinterChange('printerTarget', event.target.value)}
                    >
                      ...
                      {printerServiceStatus?.printers?.map((printer) => (
                        <option key={printer.name} value={printer.name}>
                          {printer.name} {printer.isDefault ? '(Default)' : ''} {printer.isInteractive ? '(PDF/File)' : ''}
                        </option>
                      ))}
                    </select>
                    <span className="text-xs font-normal text-muted-foreground">
                      Select your physical thermal receipt printer or use the Windows system default.
                    </span>
                  </label>
                  ...
                  <label className="flex cursor-pointer items-start justify-between gap-4 rounded-xl border border-border bg-card p-4 transition-colors hover:bg-accent/40">
                    <span className="space-y-1">
                      ...
                      <span className="block text-sm text-muted-foreground">
                        When enabled, Q Cafe sends receipts directly to the printer without print preview. If direct print is unavailable or uses an interactive printer, it falls back to print preview.
                      </span>
                    </span>
                    <input
                      type="checkbox"
