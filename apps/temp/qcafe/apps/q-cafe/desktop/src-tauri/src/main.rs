...
#[cfg(target_os = "windows")]
fn print_raw_receipt_win32(printer_name: &str, document_name: &str, receipt: &str) -> Result<u32, String> {
    ...
    let doc_wide = wide(document_name);
    let raw_data_type = wide("RAW");
    let mut printer: windows_sys::Win32::Foundation::HANDLE = std::ptr::null_mut();
    unsafe {
        if OpenPrinterW(printer_wide.as_ptr(), &mut printer, std::ptr::null()) == 0 {
            return Err(format!("Could not open printer '{}' (Windows error {}).", printer_name, GetLastError()));
        }
        let document = DOC_INFO_1W {
            pDocName: doc_wide.as_ptr() as *mut _,
        ...
        if job_id == 0 {
            ...
            return Err(format!("Could not start print job on '{}' (Windows error {}).", printer_name, GetLastError()));
        }
        if StartPagePrinter(printer) == 0 {
            EndDocPrinter(printer);
            ClosePrinter(printer);
            return Err(format!("Could not start print page on '{}' (Windows error {}).", printer_name, GetLastError()));
        }
        let payload = raw_receipt_bytes(receipt);
        ...
        if written_ok == 0 || written as usize != payload.len() {
            return Err(format!("Could not send complete receipt to '{}' (Windows error {}).", printer_name, GetLastError()));
        }
        Ok(job_id)
    }
}
...
fn printer_service_status() -> PrinterServiceStatus {
    ...
    match result {
        Some(status) => {
            let printers_list = status.printers.unwrap_or_default().into_iter().map(|p| PrinterInfo {
                name: p.name,
            ...
            let connected = status.spooler_running && (!printers_list.is_empty() || status.default_printer.is_some());
            let message = if !status.spooler_running {
                ...
                "Q Cafe Direct & Raw Print Service is connected and ready.".to_string()
            } else {
                "Windows print spooler is active. Direct printing is available.".to_string()
            };
...
fn is_interactive_printer_port(port: Option<&str>) -> bool {
    matches!(port.map(str::trim).map(str::to_ascii_uppercase).as_deref(), Some("PORTPROMPT:"))
}
...
fn raw_print_service_binary() -> Result<PathBuf, String> {
    ...
    Err("Q Cafe Raw Print Service files are missing. Run the Q Cafe installer again.".to_string())
}
...
fn raw_print_service_installer() -> Result<PathBuf, String> {
    ...
    Err("Q Cafe print-service installer files are missing. Run the Q Cafe installer again.".to_string())
}
...
fn qcafe_install_printer_service() -> Result<(), String> {
    if MessageDialog::new()
        ...
        .set_title("Install Q Cafe Raw Print Service")
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let node_wide = node.as_os_str().encode_wide().chain(Some(0)).collect::<Vec<_>>();
        ...
        let directory_wide = directory.as_os_str().encode_wide().chain(Some(0)).collect::<Vec<_>>();
...
fn qcafe_print_raw_receipt(request: RawReceiptRequest) -> Result<DirectPrintResult, String> {
    ...
    let printer = match request.printer_name.as_deref() {
        ...
        _ => status.default_printer.ok_or_else(|| "No Windows default printer is selected.".to_string())?,
    };

    let is_interactive = status
        .printers
        .iter()
        .find(|p| p.name.eq_ignore_ascii_case(&printer))
        .map(|p| p.is_interactive)
        .unwrap_or_else(|| is_interactive_printer_port(status.default_printer_port.as_deref()));

    if is_interactive {
        return Err(format!(
            "INTERACTIVE_PRINTER: Printer '{}' prompts for a file location. Use standard print preview instead.",
            printer
        ));
    }
    ...
    #[cfg(target_os = "windows")]
    {
        match print_raw_receipt_win32(&printer, &request.document_name, &request.receipt) {
            Ok(job_id) => Ok(DirectPrintResult {
                queued: true,
                job_id: Some(job_id),
                printer,
                message: "Receipt sent directly to printer.".to_string(),
            }),
            Err(win32_err) => {
                // Secondary fallback: if external service happens to be running, try TCP service
                if status.service_running {
                    if let Ok(addr) = PRINT_SERVICE_ADDRESS.parse() {
                        if let Ok(mut stream) = TcpStream::connect_timeout(&addr, Duration::from_secs(2)) {
                            if let Ok(payload) = serde_json::to_vec(&RawPrintServiceRequest {
                                printer_name: printer.clone(),
                                document_name: request.document_name,
                                receipt: request.receipt,
                            }) {
                                if stream.write_all(&payload).is_ok() && stream.write_all(b"\n").is_ok() {
                                    let mut response = String::new();
                                    if stream.read_to_string(&mut response).is_ok() {
                                        if let Ok(result) = serde_json::from_str::<RawPrintServiceResponse>(response.trim()) {
                                            if result.ok {
                                                return Ok(DirectPrintResult {
                                                    queued: true,
