...
func (app *App) preview(w http.ResponseWriter, r *http.Request) {
	...
	repository := taskRepository(app.config, id)
	...
	index := filepath.Join(repository, "index.html")
	if info, err := os.Stat(index); err == nil && !info.IsDir() {
		pathPrefix := prefix + id + "/"
		http.StripPrefix(pathPrefix, http.FileServer(http.Dir(repository))).ServeHTTP(w, r)
		return
	}

	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	if err := previewTemplate.Execute(w, task); err != nil {
		http.Error(w, fmt.Sprintf("preview rendering failed: %v", err), http.StatusInternalServerError)
	}
}
