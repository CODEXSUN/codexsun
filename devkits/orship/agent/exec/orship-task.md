...
The agent skill file is `../skills.md`.

The task file is `orship-task.md`.
...
Keep Orship product code inside `../..`.
...
Do not create `node_modules` inside `../..`.

Do not create local `dist` or `.turbo` folders inside `../..`.
...
The Go Docker manager lives in `../../docker` and runs as a private service in the local compose stack.
...
- Orship work stays inside `../..` unless a reviewed contract change exists.
...
- The Orship container scaffold lives in `../../.container`.
- `docker-compose.yml` builds separate API and web targets from one Dockerfile.
- The API container owns the SQLite path at `/workspace/storage/devkits/orship/private/data/orship_db.sqlite`.
...
- `../../VERSION` owns the Orship development version; the root `package.json` owns the global repository release version.
- The web status bar shows the Orship development version at its right edge.
- `orship-update.sh` reads `../../VERSION`, rebuilds, and recreates the local Docker stack for live updates.
...
- The Go Docker manager includes the CXApp-derived MariaDB sample lifecycle. It creates `orship-mariadb-sample` on the external `codexsun-network`, with install, drop, and reinstall routes exposed through the Orship API. The sample definition lives in `../../docker/samples/mariadb.json` and uses host port `3308`.
