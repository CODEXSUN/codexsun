# Zetro2 Implementation Task Register
...
Runtime development checkout, editor, agent, and previews run inside a private Docker environment.
...
- [ ] 2.9b Adapt Zbrowser source into the Zetro2 owner and wire preview navigation to the authenticated API.
- [ ] 2.9c Provision a persistent Docker-hosted CODEXSUN checkout shared by editor, agent, and registered development targets.
- [ ] 2.9d Keep installed control-service code and credentials separate from the editable development checkout.
...
- [ ] 3.7a Route editor, preview HTTP, WebSocket, and HMR access through private authenticated ingress.
- [ ] 3.7b Isolate preview origins and cookies from control UI sessions; keep internal service ports unpublished.
...
- [ ] 4.2 Refactor native chat and inline chat through the Zetro2 bridge.
...
- [ ] 5.11a Define the versioned daily-coding profile from roaming-coding.md with allow, deny, and critical-approval actions.
- [ ] 5.11b Bind trusted commands to executable, arguments, directory, environment, and script/configuration digests.
- [ ] 5.11c Prove non-root mounts, egress, resource limits, and tool policy prevent shell/script bypass and control-plane access.
...
- [ ] 6.5 Persist Zetro2 authoritative state and checkpoint references using module-owned repositories.
...
- [ ] 6.10 Integration checkpoint: direct-model repair, revocation, approved task.md sequencing, checkbox synchronization, and restart recovery pass.
- [ ] 6.11 Authorize bounded coding sessions and execute routine reads, edits, checks, repairs, and previews without repeated permission prompts.
- [ ] 6.12 Add durable critical requests, action-scoped approval, deduplication, expiry, denial, and state revalidation before resume.
- [ ] 6.13 Gate unattended backend mode on sandbox conformance; keep unsupported backends visibly unavailable for unattended execution.
- [ ] 6.14 Exit: approved routine work generates zero prompts while critical effects require the expected approval.
...
- [ ] 6A.9a Add device/session listing, revocation, renewal, and scoped critical-decision controls through the identity boundary.
- [ ] 6A.9b Implement the durable approval inbox and NotificationProvider with outbox delivery, deduplication, retries, and minimal payloads.
- [ ] 6A.9c Add optional browser-push enrollment and verify delivery support on an actual target mobile device/browser.
- [ ] 6A.9d Default notifications to critical decisions and actionable failures; make completion alerts opt-in and routine progress dashboard-only.
- [ ] 6A.9e Resolve concurrent approve/deny actions atomically; expired, changed, or unauthorized requests must not resume work.
- [ ] 6A.9f Document always-on host requirements, offline behavior, private network reachability, and notification delivery limitations.
...
- [ ] 7.4 Map OpenCode sessions, messages, tool activity, permissions, and errors to Zetro2 contracts.
- [ ] 7.5 Preserve Zetro2 conversation history with backend session IDs and event provenance.
...
- [ ] 12.11 Upgrade the customized editor source and adapters and run compatibility tests against the maintained source-change map.
...
- [ ] 12.15u Verify web planning → approved task/prompt → agent code change → Zbrowser dev preview → human verification → next run.
- [ ] 12.15v Prove editor, agent, tests, and preview use the same Docker checkout; verify HMR and persistent changes after restart.
- [ ] 12.15w Verify development edits cannot replace the running control service without an explicit rebuild/update operation.
- [ ] 12.15x Verify routine Docker coding completes without per-command prompts and a critical scope change creates exactly one approval request.
- [ ] 12.15y Deliver a mobile alert, authenticate, approve the exact action, and verify single resume with an attributable audit record.
- [ ] 12.15z Test offline devices, delivery failures, revocation, conflicting decisions, expired approvals, host restart, and budget exhaustion.
...
These unchecked items define the fixture. They do not change Zetro2 session policy.
...
See [source integration](../editor/README.md). No editor compile or browser test has run.
Other Phase 2 work, including runtime wiring and full branding, remains unchecked.
