# Zetro2 Automation and Governance Design
...
Use the [roaming coding profile](roaming-coding.md) to authorize routine actions once per bounded session.
The server handles routine tool decisions; critical requests enter a durable inbox with optional mobile push.
This replaces repeated IDE confirmation as the future execution path, not the current WebStorm permission policy.
...
| Proposed endpoint under /api/v1/zetro2        | Behavior                                                                       |
| --------------------------------------------- | ------------------------------------------------------------------------------ |
...
Reuse available public policy contracts and implement only Zetro2-owned accounting behavior.
