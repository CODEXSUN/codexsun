# ZCode Daily Coding Platform Plan

## 1. Outcome and status

Build ZCode as a daily coding environment with authenticated workspaces,
a customized OpenVSCode editor, and one chat for multiple models and agent backends.
...
The [original brief](readme.md) remains the source of the core engine concepts.
This plan updates its UI, identity, integration, and delivery sequence.

## 2. Verified starting point

- OpenVSCode source exists at `apps/temp/openvscode-server` inside this repository.
- Its package manifest declares version `1.110.0`. This is not a verified running version.
- Its source includes `src/vs/workbench/contrib/chat` and `inlineChat`.
- Chat contributions register language-model services and tool confirmation services.
- Its product configuration and license identify the editor source as MIT licensed.
- ZCode currently builds its editor from a pinned upstream container image.
- The existing Codexsun OS extension supplies Add-ons and Zbrowser views.
- The current demo documents localhost access without user authentication.
- The current editor and preview containers share a writable workspace volume.

Use the in-repository source for the planned copy.
Source availability does not prove that GitHub Copilot service features work in this build.

## 3. Product scope and delivery order

1. Establish identity, roles, membership, and enforceable workspace access.
2. Import and build the OpenVSCode source with controlled ZCode customization.
...
ZCode owns product behavior. Reuse public Framework and Platform Core capabilities
...
devkits/zcode/
  ...
  openvscode/                  copied, pinned upstream editor source
  editor/                     ZCode-owned editor integration and patch manifest
  .container/                 builds, runtime services, lifecycle scripts
storage/apps/private/zcode/    scoped state and artifacts through storage providers
dist/zcode/                   build and verification output
```
...
Do not silently apply vendor exceptions to ZCode-authored files.
...
Copy tracked source from `apps/temp/openvscode-server` to `devkits/zcode/openvscode`
...
Distinguish local changes from the pinned upstream baseline.

Keep four explicit custom layers:

1. Product layer: ZCode name, icons, settings, menus, and built-in extension configuration.
...
4. Runtime bridge: typed HTTP commands and resumable WebSocket events from the ZCode API.

Prefer extension points where they satisfy the required behavior.
Use small, documented workbench patches for missing hooks.
Maintain an ordered patch manifest and test patches against upstream upgrades.
Switch the editor image to the verified source-built artifact after parity tests pass.
Keep the previous image available for rollback without deleting workspace volumes.
...
Keep the ZCode API and core contracts in TypeScript.

LangGraph owns the outer engineering workflow through a ZCode `WorkflowEngine` port.
...
This is an illustrative application task, not a change to ZCode's own session policy.
...
Map backend IDs to ZCode run IDs and preserve event provenance.

ZCode owns authoritative task state, access decisions, approvals, and the audit trail.
Adapters may retain execution checkpoints referenced by ZCode.
...
Use Playwright for application browser actions and for independent ZCode E2E tests.
...
Each backend must prove that ZCode can enforce its effective permissions.
...
- A source upgrade and rollback preserve workspace data and editor settings.
...
Pin upstream editor and adapter versions. Maintain a compatibility matrix.
...
Add a ZCode-owned automation module for definitions, catalog versions, executions, and usage accounting.
...
Add a ZCode-owned Memory Bank as the structured documentation foundation before live coding.
