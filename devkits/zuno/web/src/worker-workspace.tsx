...
import { CodeIcon, MessageSquareIcon, Trash2Icon } from "lucide-react";
...
import { OpenVscodePage } from "./openvscode-page";
...
type Page = "workers" | "new" | "agents" | "editor";
...
export function WorkerWorkspace({ request, logout }: { request: typeof fetch; logout: () => void }) {
  ...
  const title = page === "new" ? "New CXForge workspace" : page === "agents" ? "ZXA work chat" : page === "editor" ? "Workspace editor" : selected?.name || "CXForge workspaces";
  return <MainWorkspace applicationId="zuno" applicationName="Zuno" showApplicationHeader applicationHeaderTitle={title} showApplicationIdentity={false} user={{ initials: "ZU", name: "Zuno client", onSignOut: logout }} workspaceTitle="Worker control" statusLabel="One workspace per container">
    <main className="h-full overflow-auto p-4 md:p-6">
      {page === "new" ? <WorkspaceCreatePage busy={busy} error={error} onBack={() => navigate("workers")} onCreate={create} onSaveProfile={saveProfile} profiles={profiles} /> : page === "agents" ? <AgentChat request={request} servers={servers} /> : page === "editor" ? <OpenVscodePage request={request} /> : <WorkerListPage command={command} error={error} onCommand={setCommand} onCreate={() => navigate("new")} onDrop={setDropCandidate} onSelect={setSelected} onShowAll={() => { setSelected(undefined); setCommand(undefined); }} operation={operation} selected={selected} servers={servers} snapshot={snapshot} snapshots={snapshots} script={script} setScript={setScript} busy={busy} onExecute={execute} />}
...
function WorkerListPage({ command, error, onCommand, onCreate, onDrop, onSelect, onShowAll, operation, selected, servers, snapshot, snapshots, script, setScript, busy, onExecute }: {
  ...
  return <>
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><Button variant="ghost" onClick={onShowAll}>All workers</Button><div className="flex gap-2"><Button aria-label="Open workspace editor" size="icon-sm" type="button" variant="outline" onClick={() => navigate("editor")}><CodeIcon /></Button><Button aria-label="Open ZXA work chat" size="icon-sm" type="button" variant="outline" onClick={() => navigate("agents")}><MessageSquareIcon /></Button><Button onClick={onCreate}>Create workspace</Button></div></div>
...
function currentPage(): Page { return window.location.hash === "#/workers/new" ? "new" : window.location.hash === "#/agents" ? "agents" : window.location.hash === "#/editor" ? "editor" : "workers"; }
function navigate(page: Page): void { window.location.hash = page === "new" ? "/workers/new" : page === "agents" ? "/agents" : page === "editor" ? "/editor" : "/overview"; }
