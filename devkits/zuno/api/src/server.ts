...
import { PortalStore } from "./modules/cxforge/portal-store";
import { PortalService } from "./modules/cxforge/portal-service";
import { registerPortalRoutes } from "./modules/cxforge/portal-routes";
import { registerWorkspaceRoutes } from "./modules/cxforge/workspace-routes";
import { ZunoHandoffService } from "./modules/handoff/handoff-service";
import { ZunoHandoffProvider } from "./modules/handoff/provider";
import { registerZetroHandoffRoutes } from "./modules/handoff/routes";
import { AgentStore } from "./modules/agents/agent-store.js";
import { AgentService } from "./modules/agents/agent-service.js";
import { registerAgentRoutes } from "./modules/agents/routes.js";
const { createZunoMcpGateway } = await import("../../mcp/server/zuno-mcp-gateway.mjs");
...
app.get("/api/v1/zuno/editor", async () => ({ configured: Boolean(config.codeServerUrl), url: config.codeServerUrl }));
...
const agentStore = new AgentStore(config.agentDatabasePath);
const agentService = new AgentService(agentStore, portalStore, config.agentLeaseKey, config.zxaControlKey);
const mcp = createZunoMcpGateway({ appMode: config.appMode, auditPath: resolve(config.dataRoot, "audit/zuno-mcp.jsonl"), repositoryRoot: config.repositoryRoot });
// Local development has one explicit CXForge worker; provisioned workers add their own records.
portalStore.saveServer({ apiUrl: config.cxforgeUrl, credential: config.cxforgeClientKey, name: "Local CXForge" }, "00000000-0000-4000-8000-000000000001");
...
registerWorkspaceRoutes(app, new PortalService(portalStore), config.repositoryRoot, (request) => identity.authenticate(request.headers.authorization, request.headers["x-codexsun-browser-session"])?.id);
registerAgentRoutes(app, agentService, (request) => identity.authenticate(request.headers.authorization, request.headers["x-codexsun-browser-session"])?.id, config.agentBootstrapKey);
app.post("/api/v1/zuno/mcp", async (request, reply) => {
  ...
  return mcp.handle(request.body, actor);
});
app.addHook("onClose", () => portalStore.close());
app.addHook("onClose", () => agentService.close());
...
function isPublicPath(url: string, method: string): boolean {
  ...
  return path === "/api/v1/zuno/auth/login"
    ...
    || (method === "POST" && path === "/api/v1/zuno/agents/register")
