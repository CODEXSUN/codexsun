...
```sh
sh devkits/cxforge/.container/cxforge-setup.sh
```
...
3. Read the release number from `../VERSION`.
...
```sh
CXFORGE_NO_CACHE=true sh devkits/cxforge/.container/cxforge-setup.sh
```
...
```sh
sh devkits/cxforge/.container/cxforge-update.sh
```
...
- The release number from `../VERSION`.
- The first 12 characters of the current Git commit SHA.
- The `-dirty` suffix when `..` has uncommitted changes.
...
```sh
sh devkits/cxforge/.container/cxforge-drop.sh
```
...
```sh
CXFORGE_CONFIRM_DROP=yes sh devkits/cxforge/.container/cxforge-drop.sh
```
...
- Setup: `../.container/cxforge-setup.sh`
- Update: `../.container/cxforge-update.sh`
- Drop: `../.container/cxforge-drop.sh`
