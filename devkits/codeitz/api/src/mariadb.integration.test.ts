...
const connectionUrl = process.env.CODEITZ_MARIADB_INTEGRATION_URL;

test("runs codeitz migrations against an explicit MariaDB test database", { skip: !connectionUrl }, async () => {
  ...
  try {
    ...
    await runner.run({ moduleId: "codeitz.integration", migrations: [], seeders: [] });
  } finally {
