...
const connectionUrl = process.env.PROJEX_MARIADB_INTEGRATION_URL;

test("runs projex migrations against an explicit MariaDB test database", { skip: !connectionUrl }, async () => {
  ...
  try {
    ...
    await runner.run({ moduleId: "projex.integration", migrations: [], seeders: [] });
  } finally {
